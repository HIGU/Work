...
class PDF_j extends MBFPDF  // 日本語PDFクラスを拡張します。
{
    ...
    function Header()   //頭につけたい内容はここに。コンストラクタみたいに自動実行されます。
    {
        ...
        */
        ...
        /// $this->Cell(80);
        // Framed title
        $list_title = '付表２-（１） 課税売上割合・控除対象仕入税額等の計算表';
        $this->Cell(80, 10, $list_title, 'TB', 0, 'C');
        $this->Ln();    // 改行
        $list_title2 = ' 〔経過措置対象課税資産の譲渡を含む課税期間用〕';
...
$data_f = array();  // スカラー変数ではなくて、配列だということを明示
    // Column titles
    $pdf->wh_usr = array('項', '', '目', '旧税率分小計Ｘ', '税率6.24％適用分Ｄ', '税率7.8％適用分Ｅ', '金額Ｆ（Ｘ+Ｄ+Ｅ）');
