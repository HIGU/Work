<?php
//////////////////////////////////////////////////////////////////////////////////
// 月次損益関係 消費税申告書 確定申告書付表1-(2) PDF出力(印刷) FPDF/MBFPDF使用  //
...
class PDF_j extends MBFPDF  // 日本語PDFクラスを拡張します。
{
    ...
    function Header()   //頭につけたい内容はここに。コンストラクタみたいに自動実行されます。
    {
        ...
        */
        ...
        /// $this->Cell(80);
        // Framed title
        $list_title = '付表１-２ 税率別消費税額計算表';
        $this->Cell(80, 10, $list_title, 'TB', 0, 'C');
        $this->Ln();    // 改行
        $list_title2 = ' 兼地方消費税の課税標準となる消費税額計算表〔経過措置対象課税資産の譲渡を含む課税期間用〕';
...
$data_f = array();  // スカラー変数ではなくて、配列だということを明示
    // Column titles
    $pdf->wh_usr = array('区', '', '分', '税率3％適用分Ａ', '税率4％適用分Ｂ', '税率6.3％適用分Ｃ', '合計 Ｘ（Ａ+Ｂ+Ｃ）');
