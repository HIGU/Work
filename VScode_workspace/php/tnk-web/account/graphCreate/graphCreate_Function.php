...
function getGraphData($session, $result, $plot)
{
    ...
    ///// プロット項目取得
    if ($session->get_local($plot) <= 36) {     // 製造経費
        $query = "
...
function graphCreateExecute($session, $result, $graph_name, $plot1, $plot2='')
{
    ...
    $graph = new Graph(680, 350, 'auto');       // グラフの大きさ X/Y
    ...
    if ($session->get_local($plot1) != 'blank' && $session->get_local($plot2) != 'blank') {
        $title = getItemArrayKey($session->get_local($plot1)) . 'と' . getItemArrayKey($session->get_local($plot2));
    } else {
