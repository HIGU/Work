...
function ExecuteDecision()
{
    ...
    $Summary['plan_num']      = 0;
    $Summary['plan_stop']     = 0;   // 刃物切れ時間→計画停止（予備１）2010/05/25 ADD
    $Summary['priority_stop'] = 0;   // 一時停止時間→優先対応停止（予備２）2010/05/25 ADD
    for ($i=0;$i<$LogNum;$i++) {
        ...
        if ($CsvFlg == 1) {
            ...
            switch ($Report['MacState'][$i]) {
                ...
                case '10':
                    // 計画停止
                    $Summary['plan_stop'] += CalWorkTime($Report['FromDate'][$i],$Report['FromTime'][$i],$Report['ToDate'][$i],$Report['ToTime'][$i]);
                    break;
                case '11':
                    // 優先対応停止
                    $Summary['priority_stop'] += CalWorkTime($Report['FromDate'][$i],$Report['FromTime'][$i],$Report['ToDate'][$i],$Report['ToTime'][$i]);
                    break;
                case '1':
                    // 自動運転
                    // 以下のrunning_time は 自動＋無人 で本稼働時間
    ...
    $sql = ' insert into equip_upload_summary values('
         ...
         . (int)$ProcessingDate                                     . ','
         . (int)$Summary['plan_stop']                               . ','
         . (int)$Summary['priority_stop']                           . ')';
    if (!pg_query($con,$sql)) {
        MLog($sql);
        pg_query ($con , 'ROLLBACK');
        $SYSTEM_MESSAGE = "データベースの更新に失敗しました\n$sql".var_dump($Report);
