<?php
//////////////////////////////////////////////////////////////////////////////
// 設備稼働管理用 グラフ Create Class                                       //
// Copyright (C) 2004-2005 Kazuhiro.Kobayashi tnksys@nitto-kohki.co.jp      //
// Changed history                                                          //
// 2004/08/11 Created   EquipGraphClass.php                                 //
//            jpGraph-1.16 base created                                     //
...
// 2004/08/18 ページ制御 graph_page backward forward state_summary()追加    //
// 2004/08/23 jpgraph.php -> jpgraph_equip.php に変更してY軸の(-)表示削除   //
// 2004/08/28 Ver1.01 interval プロパティを追加しset_xtime('max')をサポート //
// 2004/08/29 Ver1.02 xtimeArrValプロパティを追加しout_select_xtime()を追加 //
// 2004/09/01 Ver1.03 out_graph()にオプションでサマリー結果を書込む機能追加 //
// 2004/09/10 直前のwork_cntとmax_work_cntを比較($max_work_cnt < $res[0][1])//
//            Ver1.04 yaxis_min()に与えるデータを決定するように変更         //
// 2005/05/20 db_connect() → funcConnect() へ変更 pgsql.phpで統一のため    //
// 2005/08/20 php5 へ移行  Ver1.05                                          //
// 2005/09/28 generate_data()メソッドでmax(work_cnt)1191の時にindexが使われ //
//  Ver1.06   ない場合があるため"begin; SET enable_seqscan TO 'off';"を追加 //
//////////////////////////////////////////////////////////////////////////////
...
class EquipGraph
{
    ...
    ************************************************************************/
    ...
    function EquipGraph($mac_no, $siji_no, $koutei, $sampling=180)
    {
        if (!isset($_SESSION)) {                    // セッションの開始チェック
            session_start();                        // Notice を避ける場合は頭に@
        }
        if ($this->set_condition($mac_no, $siji_no, $koutei) == false) {
            $addr_status = "ErrorEquipGraphPage.php?status=1&mac_no=$mac_no&siji_no=$siji_no&koutei=$koutei";
    ...
    function set_condition($mac_no, $siji_no, $koutei)
    {
        if ($mac_no  == '') return false;
        if ($siji_no == '') return false;
    ...
    ***************************************************************************/
    ...
    // グラフ化する終了時間の設定
    protected function set_graph_endTime($strTime, $xtime)
    {
    ...
    protected function generate_data()
    {
        ...
        $query = " select work_cnt
                    from
                        equip_work_log2
                    where
                        equip_index(mac_no, siji_no, koutei, date_time) >= '{$this->mac_no}{$this->siji_no}{$this->koutei}{$this->graph_strTime}'
                    and
                        equip_index(mac_no, siji_no, koutei, date_time) <= '{$this->mac_no}{$this->siji_no}{$this->koutei}{$this->graph_endTime}'
                    offset 0 limit 1
        ";
        ...
        if ($empty == false) {
            ...
            $query = " select work_cnt
                        from
                            equip_work_log2
                        where
                            equip_index(mac_no, siji_no, koutei, date_time) > '{$this->mac_no}{$this->siji_no}{$this->koutei}00000000000000'
                        and
                            equip_index(mac_no, siji_no, koutei, date_time) < '{$this->mac_no}{$this->siji_no}{$this->koutei}{$this->graph_strTime}'
                        order by
        ...
        while (1) {
            $query = " select mac_state
                        ...
                        where
                            equip_index(mac_no, siji_no, koutei, date_time) >= '{$this->mac_no}{$this->siji_no}{$this->koutei}{$this->graph_strTime}'
                        and
                            equip_index(mac_no, siji_no, koutei, date_time) <= '{$this->mac_no}{$this->siji_no}{$this->koutei}{$now_time}'
                        order by
                            equip_index(mac_no, siji_no, koutei, date_time) DESC
                        offset 0 limit 1
            ";
