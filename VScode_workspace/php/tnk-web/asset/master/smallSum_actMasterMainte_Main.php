    ...
    if (getCheckAuthority(22)) {                    // 認証チェック
        ...
        if ( getResult($query, $res_chk) > 0 ) {    // 登録あり UPDATE 更新
            ...
            if (query_affected($query) <= 0) {
                $_SESSION['s_sysmsg'] .= "管理部門コード：{$code_act} の管理部門名を{$name_act}へ変更失敗！";      // .= に注意
                $msg_flg = 'alert';
                return false;
            } else {
                $_SESSION['s_sysmsg'] .= "管理部門コード：{$code_act}の 管理部門名を{$name_act}に変更しました！"; // .= に注意
                return true;
            }
        } else {                                    // 登録なし INSERT 新規   
            ...
            if (query_affected($query) <= 0) {
                $_SESSION['s_sysmsg'] .= "管理部門コード：{$code_act} 管理部門名：{$name_act}の追加に失敗！";      // .= に注意
                $msg_flg = 'alert';
                return false;
            } else {
                $_SESSION['s_sysmsg'] .= "管理部門コード：{$code_act} 管理部門名：{$name_act}を追加しました！";    // .= に注意
                return true;
            }
        }
    } else {                                        // 権限なしエラー
...
function groupMaster_del($request)
{
    if (getCheckAuthority(22)) {    // 認証チェック
        ...
        if (getResult($query, $res_chk) > 0) {    // 登録あり
            $_SESSION['s_sysmsg'] .= "この管理部門コードはすでに他のマスターで使用されています！";
            return false;
        }
        $query = sprintf("DELETE FROM smallsum_assets_actname_master WHERE code_act = %d", $code_act);
        if (query_affected($query) <= 0) {
            $_SESSION['s_sysmsg'] .= "管理部門コード：{$code_act} 管理部門名：{$name_act}の削除に失敗！";   // .= に注意
            $msg_flg = 'alert';
            return false;
        } else {
            $_SESSION['s_sysmsg'] .= "管理部門コード：{$code_act} 管理部門名：{$name_act}を削除しました！"; // .= に注意
            return true;
        }
    } else {                        // 権限なしエラー
        $_SESSION['s_sysmsg'] .= "編集権限がありません。必要な場合には、担当者に連絡して下さい。";
...
{
    $query_g = "
        SELECT  groupm.code_act              AS 管理部門コード   -- 0
            ,   groupm.name_act              AS 管理部門名       -- 1
        FROM
