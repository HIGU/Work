...
<?php
//////////////////////////////////////////////////////////////////////////////
// 平出工場棚番の更新 手動で処理用コマンドライン版                          //
// AS/400 ----> Web Server (PHP)                                            //
// Copyright (C) 2012-     Norihisa.Ohya norihisa_ooya@nitto-kohki.co.jp    //
// Changed histoy                                                           //
// 2012/01/13 Created  hiraide_invent_get_ftp.php                           //
// 2012/02/20 平出棚卸用データも同時に更新するように変更                    //
//////////////////////////////////////////////////////////////////////////////
...
fwrite($fpb, "平出工場棚番の更新\n");
fwrite($fpb, "/var/www/html/system/daily/hiraide_invent_get_ftp.php \n");

//////////////////////////// 文字化けする為FTP接続は使用しない
// FTPのターゲットファイル
//define('MSHGNM', 'UKWLIB/W#MSHGNM');        // 製品グループコードマスターファイル
//define('W_MSHGNM', '/var/www/html/system/backup/W#MSHGNM.TXT');  // 製品グループコードマスターのDownloadファイル
...
$file_name  = '/home/guest/daily/W#HIRATANA.CSV';
$file_temp  = '/home/guest/daily/Q#HIRATANA.tmp';
$file_write = '/home/guest/daily/Q#HIRATANA.txt';
...
// 製品グループコードマスター 日報処理
$file_name = '/home/guest/daily/Q#HIRATANA.txt';
$file_name_bak = '/home/guest/daily/Q#HIRATANA-bak.txt';
if (file_exists($file_name)) {            // ファイルの存在チェック
    ...
    if ( ($fp = fopen($file_name, 'r')) ) {
        ...
        // while ($data = fgetcsv($fp, 200, "_")) {     // FTP接続用
            ...
            ///////// 登録済みのチェック
            $query_chk = sprintf("select tnk_tana from parts_stock_master where parts_no='%s'", $data[0]);
            if (getUniResTrs($con, $query_chk, $res_chk) <= 0) {    // トランザクション内での 照会専用クエリー
                ...
                ///// 登録あり update 使用
                $query = sprintf("update parts_stock_master set tnk_tana='%s'
                        where parts_no='%s'", $data[1], $data[0]);
                if (query_affected_trans($con, $query) <= 0) {      // 更新用クエリーの実行
        ...
        echo "Q#HIRATANA.txtをオープン出来ません\n";
        fwrite($fpa,"".$rowcsv."W#HIRATANA.txtをオープン出来ません\n");
        fwrite($fpb,"".$rowcsv."W#HIRATANA.txtをオープン出来ません\n");
    }
    /**********
...
// メッセージを返す
if ($flag2==1) {
    echo "$log_date 平出工場棚番の更新: $rec_ok/$rowcsv 件登録しました。\n";
    echo "$log_date 平出工場棚番の更新: {$row_in}/{$rowcsv} 件 追加 \n";
    echo "$log_date 平出工場棚番の更新: {$row_up}/{$rowcsv} 件 変更 \n";
    fwrite($fpa, "$log_date 平出工場棚番の更新: $rec_ok/$rowcsv 件登録しました。\n");
    fwrite($fpa, "$log_date 平出工場棚番の更新: {$row_in}/{$rowcsv} 件 追加 \n");
    fwrite($fpa, "$log_date 平出工場棚番の更新: {$row_up}/{$rowcsv} 件 変更 \n");
    fwrite($fpb, "$log_date 平出工場棚番の更新: $rec_ok/$rowcsv 件登録しました。\n");
    fwrite($fpb, "$log_date 平出工場棚番の更新: {$row_in}/{$rowcsv} 件 追加 \n");
    fwrite($fpb, "$log_date 平出工場棚番の更新: {$row_up}/{$rowcsv} 件 変更 \n");
} else {
    echo "{$log_date} 平出工場棚番の更新データがありません。\n";
    fwrite($fpa, "$log_date 平出工場棚番の更新データがありません。\n");
    fwrite($fpb, "$log_date 平出工場棚番の更新データがありません。\n");
}
...
// 製品グループコード 日報処理 準備作業
// $file_name = '/home/www/html/weekly/Q#MIITEM.CSV';
$file_name  = '/home/guest/daily/W#HIRATAN2.CSV';
$file_temp  = '/home/guest/daily/Q#HIRATAN2.tmp';
$file_write = '/home/guest/daily/Q#HIRATAN2.txt';
...
// 製品グループコードマスター 日報処理
$file_name = '/home/guest/daily/Q#HIRATAN2.txt';
$file_name_bak = '/home/guest/daily/Q#HIRATAN2-bak.txt';
if (file_exists($file_name)) {            // ファイルの存在チェック
    $rowcsv = 0;        // read file record number
    $row_in = 0;        // insert record number 成功した場合にカウントアップ
    $row_up = 0;        // update record number   〃
    ...
    if ( ($fp = fopen($file_name, 'r')) ) {
        ...
        // while ($data = fgetcsv($fp, 200, "_")) {     // FTP接続用
            ...
            //$data[1] = trim($data[1]);          // 部品名の前後のスペースを削除 AS/400のPCIXを使用したFTP転送のため
            ///////// 登録済みのチェック
            $query_chk = sprintf("select tnk_tana from parts_stock_master_hiraide where parts_no='%s'", $data[0]);
            if (getUniResTrs($con, $query_chk, $res_chk) <= 0) {    // トランザクション内での 照会専用クエリー
                ///// 登録なし insert 使用
                $query = sprintf("insert into parts_stock_master_hiraide (parts_no, abc_kubun, stock_id, tnk_tana, tanka)
                        values('%s','%s','%s','%s', '%s')", $data[0],$data[2],$data[4],$data[1],$data[3]);
                if (query_affected_trans($con, $query) <= 0) {      // 更新用クエリーの実行
                    echo "$log_date {$rowcsv}:レコード目の書込みに失敗しました!\n";
                    fwrite($fpa, "$log_date {$rowcsv}:レコード目の書込みに失敗しました!\n");
                    fwrite($fpb, "$log_date {$rowcsv}:レコード目の書込みに失敗しました!\n");
                ...
                ///// 登録あり update 使用
                $query = sprintf("update parts_stock_master_hiraide set abc_kubun='%s', stock_id='%s', tnk_tana='%s', tanka='%s'
                        where parts_no='%s'", $data[2],$data[4],$data[1],$data[3], $data[0]);
                if (query_affected_trans($con, $query) <= 0) {      // 更新用クエリーの実行
                    echo "$log_date {$rowcsv}:レコード目のUPDATEに失敗しました!\n";
                    fwrite($fpa, "$log_date {$rowcsv}:レコード目のUPDATEに失敗しました!\n");
                    fwrite($fpb, "$log_date {$rowcsv}:レコード目のUPDATEに失敗しました!\n");
        ...
        echo "Q#HIRATAN2.txtをオープン出来ません\n";
        fwrite($fpa,"".$rowcsv."W#HIRATAN2.txtをオープン出来ません\n");
        fwrite($fpb,"".$rowcsv."W#HIRATAN2.txtをオープン出来ません\n");
    }
    /**********
      ///////////////////////////////////////////////// stderr(2)→をstdout(1)へ 2>&1
    $result2 = `/usr/local/pgsql/bin/psql TnkSQL < /home/www/html/weekly/qmiitem 2>&1`;
...
// メッセージを返す
if ($flag2==1) {
    echo "$log_date 平出工場棚卸用データの更新: $rec_ok/$rowcsv 件登録しました。\n";
    echo "$log_date 平出工場棚卸用データの更新: {$row_in}/{$rowcsv} 件 追加 \n";
    echo "$log_date 平出工場棚卸用データの更新: {$row_up}/{$rowcsv} 件 変更 \n";
    fwrite($fpa, "$log_date 平出工場棚卸用データの更新: $rec_ok/$rowcsv 件登録しました。\n");
    fwrite($fpa, "$log_date 平出工場棚卸用データの更新: {$row_in}/{$rowcsv} 件 追加 \n");
    fwrite($fpa, "$log_date 平出工場棚卸用データの更新: {$row_up}/{$rowcsv} 件 変更 \n");
    fwrite($fpb, "$log_date 平出工場棚卸用データの更新: $rec_ok/$rowcsv 件登録しました。\n");
    fwrite($fpb, "$log_date 平出工場棚卸用データの更新: {$row_in}/{$rowcsv} 件 追加 \n");
    fwrite($fpb, "$log_date 平出工場棚卸用データの更新: {$row_up}/{$rowcsv} 件 変更 \n");
} else {
    echo "{$log_date} 平出工場棚卸用データの更新データがありません。\n";
    fwrite($fpa, "$log_date 平出工場棚卸用データの更新データがありません。\n");
    fwrite($fpb, "$log_date 平出工場棚卸用データの更新データがありません。\n");
}
