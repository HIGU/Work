...
$data = fgetcsv($fp_temp, 100, '_');    // 実レコードは93バイトなのでちょっと余裕をデリミタは'_'に注意
fclose($fp_temp);                       // 1レコード読み即終了
if ($data[12] == '') {
    fwrite($fpa, "$log_date 在庫経歴・在庫マスター AS data not record!\n");
    fwrite($fpb, "$log_date 在庫経歴・在庫マスター AS data not record!\n");
    echo "$log_date 在庫経歴・在庫マスター AS data not record!\n";
    exit();
}
///// 更新対象日で合計レコード数を取得
...
";
if (getUniResTrs($con, $query_chk, $rec_tnk) < 0) {    // トランザクション内での 照会専用クエリー
    fwrite($fpa, "$log_date 更新日:{$data[12]} : count(*) SQL error!\n");
    fwrite($fpb, "$log_date 更新日:{$data[12]} : count(*) SQL error!\n");
    echo "$log_date 更新日:{$data[12]} : count(*) SQL error!\n";
    exit();
}
...
if (file_exists($file_orign)) {         // ファイルの存在チェック
    ...
    fwrite($fpa, "$log_date 在庫経歴差分の更新 : $rec_ok/$rec 件登録しました。\n");
    fwrite($fpa, "$log_date 在庫経歴差分の更新 : {$ins_ok}/{$rec} 件 追加 \n");
    fwrite($fpa, "$log_date 在庫経歴差分の更新 : {$upd_ok}/{$rec} 件 変更 \n");
    fwrite($fpb, "$log_date TNKサーバー側レコード : $rec_tnk/$rec 件登録済みです。\n");
    fwrite($fpb, "$log_date 在庫経歴差分の更新 : $rec_ok/$rec 件登録しました。\n");
    fwrite($fpb, "$log_date 在庫経歴差分の更新 : {$ins_ok}/{$rec} 件 追加 \n");
    fwrite($fpb, "$log_date 在庫経歴差分の更新 : {$upd_ok}/{$rec} 件 変更 \n");
    echo "$log_date TNKサーバー側レコード : $rec_tnk/$rec 件登録済みです。\n";
    echo "$log_date 在庫経歴差分の更新 : $rec_ok/$rec 件登録しました。\n";
    echo "$log_date 在庫経歴差分の更新 : {$ins_ok}/{$rec} 件 追加 \n";
    echo "$log_date 在庫経歴差分の更新 : {$upd_ok}/{$rec} 件 変更 \n";
} else {
