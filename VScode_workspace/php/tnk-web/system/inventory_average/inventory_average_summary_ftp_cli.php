...
<?php
//////////////////////////////////////////////////////////////////////////////
// 資材在庫部品 保有月等のサマリーファイル (SIDZKIL4) 取込 処理用 FTP CLI版 //
// AS/400 ----> Web Server (PHP)                                            //
// Copyright (C) 2007-2010 Kazuhiro.Kobayashi tnksys@nitto-kohki.co.jp      //
// Changed history                                                          //
// 2007/05/18 Created  inventory_average_summary_ftp_cli.php                //
...
fwrite($fpb, "資材在庫サマリーの自動更新処理実行\n");
fwrite($fpb, "/var/www/html/system/inventory_average/inventory_average_summary_ftp_cli.php\n");
echo "/var/www/html/system/inventory_average/inventory_average_summary_ftp_cli.php\n";
...
if ($ftp_stream = ftp_connect(AS400_HOST)) {
    if (ftp_login($ftp_stream, AS400_USER, AS400_PASS)) {
        ...
        if (ftp_get($ftp_stream, $save_file, $target_file, FTP_ASCII)) {
            ...
            fwrite($fpb, "$log_date 資材在庫サマリー ftp_get download OK " . $target_file . '→' . $save_file . "\n");
            echo "$log_date 資材在庫サマリー ftp_get download OK " . $target_file . '→' . $save_file . "\n";
            $ftp_flg = true;
        } else {
            $log_date = date('Y-m-d H:i:s');
            fwrite($fpa, "$log_date 資材在庫サマリー ftp_get() error " . $target_file . "\n");
            fwrite($fpb, "$log_date 資材在庫サマリー ftp_get() error " . $target_file . "\n");
            echo "$log_date 資材在庫サマリー ftp_get() error " . $target_file . "\n";
        }
    } else {
        $log_date = date('Y-m-d H:i:s');
        fwrite($fpa, "$log_date 資材在庫サマリー ftp_login() error \n");
        fwrite($fpb, "$log_date 資材在庫サマリー ftp_login() error \n");
        echo "$log_date 資材在庫サマリー ftp_login() error \n";
    }
    ftp_close($ftp_stream);
} else {
    $log_date = date('Y-m-d H:i:s');
    fwrite($fpa, "$log_date 資材在庫サマリー ftp_connect() error \n");
    fwrite($fpb, "$log_date 資材在庫サマリー ftp_connect() error \n");
    echo "$log_date 資材在庫サマリー ftp_connect() error \n";
}
if (!$ftp_flg) {
...
if ($con = db_connect()) {
    ...
    fwrite($fpb, "$log_date 資材在庫サマリー db_connect() error \n");
    echo "$log_date 資材在庫サマリー db_connect() error \n";
    exit();
}
///// 準備作業
...
if (file_exists($file_orign)) {         // ファイルの存在チェック
    ...
    $log_date = date('Y-m-d H:i:s');
    fwrite($fpa, "$log_date 資材在庫サマリー 前々日分のデータを $delRec 件 削除完了 \n");
    fwrite($fpb, "$log_date 資材在庫サマリー 前々日分のデータを $delRec 件 削除完了 \n");
    echo "$log_date 資材在庫サマリー 前々日分のデータを $delRec 件 削除完了 \n";
    ...
    while (($data = fgetcsv($fp, 100, '_')) !== FALSE) {    // デリミタはアンダーバーに注意
        $rec++;
        ...
        for ($f=0; $f<$num; $f++) {
            $data[$f] = mb_convert_encoding($data[$f], 'UTF-8', 'AUTO');       // SJISをEUC-JPへ変換
        ...
        if (getUniResTrs($con, $query_chk, $res_chk) <= 0) {    // トランザクション内での 照会専用クエリー
            ...
            if (query_affected_trans($con, $query) <= 0) {      // 更新用クエリーの実行
                ...
                fwrite($fpb, "$log_date 部品番号:{$data[1]} : {$rec}:レコード目の書込みに失敗しました!\n");
            ...
            if (query_affected_trans($con, $query) <= 0) {      // 更新用クエリーの実行
                ...
                fwrite($fpb, "$log_date 部品番号:{$data[1]} : {$rec}:レコード目のUPDATEに失敗しました!\n");
                // query_affected_trans($con, "rollback");     // transaction rollback
                $rec_ng++;
                ////////////////////////////////////////// Debug start
                for ($f=0; $f<$num; $f++) {
    ...
    fwrite($fpb, "$log_date 資材在庫サマリーの更新 : $rec_ok/$rec 件登録しました。\n");
    fwrite($fpb, "$log_date 資材在庫サマリーの更新 : {$ins_ok}/{$rec} 件 追加 \n");
    fwrite($fpb, "$log_date 資材在庫サマリーの更新 : {$upd_ok}/{$rec} 件 変更 \n");
