...
class over_time_work_report_Controller
{
    ...
    ************************************************************************/
    ...
    public function display($menu, $request, $result, $model)
    {
        ...
        $v_type = $request->get('ddlist_v_type'); // 表示種類を取得
        $v_early = "";
        $v_normal = "";
        if( $v_type == "0" ) {
            $v_early = " selected";
...
//                    $now_dt  = new DateTime("20211008");            // TEST 現在日時
                    ...
                    if( $v_early ) $time_limit = '08:30';           // 取消可能時間【早出】
//                    $time_limit = '12:15';                          // TEST 取消可能時間
            ...
            case 'Results':   // 照会（結果） 画面
                ...
                $menu->set_retPOST('ddlist_v_type', $request->get('ddlist_v_type'));
