...
// 2004/02/17 by k.kobayashi $EUC2SJIS = false --> true
// 2004/02/19 by k.kobayashi Function Text override for Encode Change.
//                           Function SetTitle Added  Encode Change(Unicode).
//                           Function SetSubject Added  Encode Change(Unicode).
//                           Function SetCreator Added  Encode Change(Unicode).
//                           Function SetAuthor Added  Encode Change(Unicode).
//                           Function _unicode SJIS Check Add
//                           Function _unicode Change "auto"-->"SJIS"
// 2004/03/01 by k_kobayashi $this->_out('/CIDSystemInfo <</Registry (Adobe) /Ordering ('.$font['registry']['ordering'].') /Supplement '.$font['registry']['supplement'].'>>');
...
//                           function _putType0($font) MBFPDF Protection support.
//-------------------------------------------------------------------------
...
require_once ('fpdf_protection.php');   // Extends Protection Class 2004/03/01 Added k.kobayashi
...
class MBFPDF extends FPDF_Protection    // FPDF --> FPDF_Protection Changed k_kobayashi
{
    ...
    if (function_exists('mb_detect_encoding')) {
        if (mb_detect_encoding($txt) != "ASCII") {
            if ($EUC2SJIS && function_exists('mb_convert_encoding')) {
                $txt = chr(254).chr(255).mb_convert_encoding($txt,"UTF-16","SJIS"); // "auto"-->"SJIS" k.kobayashi 2004/02/19
            } else {
...
// 2004/02/19 Add Function SetTitle Multi-Byte Encoding(Unicode) by k.kobayashi
function SetTitle($title)
{
    ...
    $title = $this->_unicode($title);
    parent::SetTitle($title);
}

// 2004/02/19 Add Function SetSubject Multi-Byte Encoding(Unicode) by k.kobayashi
function SetSubject($title)
{
    ...
    $title = $this->_unicode($title);
    parent::SetSubject($title);
}

// 2004/02/19 Add Function SetCreator Multi-Byte Encoding(Unicode) by k.kobayashi
function SetCreator($title)
{
    // For Japanese Encode Change
    global $EUC2SJIS;
    if ($EUC2SJIS && function_exists('mb_convert_encoding')) {
    ...
    $title = $this->_unicode($title);
    parent::SetCreator($title);
}

// 2004/02/19 Add Function SetAuthor Multi-Byte Encoding(Unicode) by k.kobayashi
function SetAuthor($title)
{
    // For Japanese Encode Change
    global $EUC2SJIS;
    if ($EUC2SJIS && function_exists('mb_convert_encoding')) {
...
// Function Text override for Encode Change. 2004/02/19 by k.kobayashi
...
function _putType0($font)
{
  ...
  // 2004/03/01 Changed by k_kobayashi
  $this->_out('/CIDSystemInfo <</Registry '.$this->_textstring('Adobe').' /Ordering '.$this->_textstring($font['registry']['ordering']).' /Supplement '.$font['registry']['supplement'].'>>');
