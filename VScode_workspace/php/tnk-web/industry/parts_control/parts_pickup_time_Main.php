...
// Changed history                                                          //
// 2005/09/12 Created   parts_pickup_time_Main.php                          //
...
require_once ('parts_pickup_time_Controller.php');  // MVC の Controller部
require_once ('parts_pickup_time_Model.php');       // MVC の Model部
...
function main()
{
    ...
    ////////////// サイト設定
    $menu->set_site(INDEX_INDUST, 2);           // site_index=INDEX_INDUST(生産メニュー) site_id=2(資材部品出庫メニュー)999(サイトを開く)
    ////////////// リターンアドレス設定
    // $menu->set_RetUrl(EQUIP_MENU);              // 通常は指定する必要はない
    
    //////////// タイトル名(ソースのタイトル名とフォームのタイトル名)
    $menu->set_title('資材 部品 出庫 メニュー');
    //////////// 呼出先のaction名とアドレス設定
    ...
    //////////// ビジネスモデル部のインスタンス生成
    $model = new PartsPickupTime_Model($request);
    
    //////////// コントローラー部のインスタンス生成
    $controller = new PartsPickupTime_Controller($menu, $request, $result, $model);
