<?php
//////////////////////////////////////////////////////////////////////////////
// 組立ラインのカレンダー メンテナンス                   MVC Controller 部  //
// Copyright (C) 2006-2007 Kazuhiro.Kobayashi tnksys@nitto-kohki.co.jp      //
// Changed history                                                          //
// 2006/07/11 Created   assembly_calendar_Controller.php                    //
...
class AssemblyCalendar_Controller
{
    ///// Private properties
    ...
    ************************************************************************/
    ...
    public function __construct($menu)
    {
        ...
        //////////// カレンダーのインスタンスを登録
        for ($i=0; $i<12; $i++) {
            $this->calendar[$i] = new AssemblyCalendar();
        }
        
        //////////// ビジネスモデル部のインスタンスを生成しプロパティへ登録
        $this->model = new AssemblyCalendar_Model($this->request, $this->menu);
        
        //////////// ブラウザーのキャッシュ対策用
        $this->uniq = $this->menu->set_useNotCache('assembly_calendar');
    }
    ...
    public function Execute()
    {
        ...
        case 'Change':                                      // 組立ラインの稼働日・停止日トグル切替
        ...
        case 'TimeList':                                    // 詳細編集用リスト(１ヶ月)取得
            ...
            break;
        case 'bdDetailSave':                                // 詳細編集より稼働日／停止日の切替
            $this->model->changeHoliday($this->request, $this->result, $this->menu);
            if ($this->request->get('showMenu') == '') $this->request->add('showMenu', 'TimeEdit');
            break;
        case 'bdCommentSave':                               // 詳細編集より稼働日／停止日のコメント変編集
            $this->model->commentSave($this->request);
            if ($this->request->get('showMenu') == '') $this->request->add('showMenu', 'TimeEdit');
            break;
    ...
    ***************************************************************************/
    ...
    private function InitTargetLine($request, $session)
    {
        ...
        if ($targetLine == '') {
            if ($session->get_local('targetLine') == '') {
                $targetLine = '0000';                           // 指定がない場合は共有ライン
            } else {
    ...
    private function InitTargetCalendar($request, $session)
    {
        ...
        if ($targetCalendar == '') {
            if ($session->get_local('targetCalendar') == '') {
                // $targetCalendar = 'BDSwitch';                       // 指定がない場合は稼働日と停止日の切替
    ...
    {
        require_once ('assembly_calendar_ViewCondForm.php');
        return true;
    }
    ...
    private function ViewCalendarExecute($menu, $request, $model, $calendar, $uniq)
    {
        ...
        require_once ('assembly_calendar_ViewCalendar.php');
        return;
    }
    ...
    {
        require_once ('assembly_calendar_ViewEditComment.php');
        return true;
    }
    ...
    {
        require_once ('assembly_calendar_ViewList.php');
        return true;
    }
    
    /***** display()の Private methods 処理 *****/
    ///// 稼働時間と停止時間の編集用 Window表示
    private function ViewTimeEditExecute($menu, $request, $result, $model, $uniq)
    {
        require_once ('assembly_calendar_ViewTimeEdit.php');
        return true;
    }
    
} // class AssemblyCalendar_Controller End
