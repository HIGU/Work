<?php
//////////////////////////////////////////////////////////////////////////////
// 刻印管理システム 検索メニュー                         MVC Controller 部  //
// Copyright (C) 2007      Kazuhiro.Kobayashi tnksys@nitto-kohki.co.jp      //
// Changed history                                                          //
// 2007/11/14 Created   punchMark_search_Controller.php                     //
// 2007/11/15 リクエストがなくてページ復元以外ならのコメントを追加          //
//////////////////////////////////////////////////////////////////////////////
...
class PunchMarkSearch_Controller
{
    ///// Private properties
    ...
    ************************************************************************/
    ...
    public function __construct($menu)
    {
        ...
        $this->model = new PunchMarkSearch_Model();
    }
    ...
    {
        //////////// ブラウザーのキャッシュ対策用
        $uniq = $this->menu->set_useNotCache('punchMarkSearch');
        ...
        case 'Comment':                                     // 別ウィンドウでコメントの照会・編集
            ...
            require_once ('punchMark_search_ViewEditComment.php');
            break;
        }
    }
    ...
    ***************************************************************************/
    ...
    protected function Init($request, $session)
    {
        ...
        // 刻印コードの処理
        $this->InitTargetPunchMark_code($request, $session);
        // 棚番の処理
        $this->InitTargetShelf_no($request, $session);
        // 刻印内容の処理
        $this->InitTargetMark($request, $session);
        // 形状コードの処理
        $this->InitTargetShape_code($request, $session);
        // 客先コードの処理
    ...
    ***************************************************************************/
    ...
    private function InitTargetPartsNo($request, $session)
    {
        if ($request->get('parts_no') == '') {
            if ($session->get_local('parts_no') == '') {
                ...
                if ($request->get('page_keep') == '') $session->add_local('parts_no', '');
            }
        } else {
            // 全角入力防止
            $request->add('parts_no', mb_convert_kana($request->get('parts_no'), 'a'));
            $session->add_local('parts_no', $request->get('parts_no'));
        }
        // エラーチェックが必要なリクエストはここに記述
    }
    
    ///// 刻印コードの取得・初期化
    private function InitTargetPunchMark_code($request, $session)
    {
        if ($request->get('punchMark_code') == '') {
            if ($session->get_local('punchMark_code') == '') {
                return;     // 指定が無い場合は何もしない
            } else {
                // リクエストがなくてページ復元以外ならセッションをクリアー
                if ($request->get('page_keep') == '') $session->add_local('punchMark_code', '');
            }
        } else {
            // 全角入力防止
            $request->add('punchMark_code', mb_convert_kana($request->get('punchMark_code'), 'a'));
            $session->add_local('punchMark_code', $request->get('punchMark_code'));
        }
        // エラーチェックが必要なリクエストはここに記述
    }
    ...
    {
        if ($request->get('shelf_no') == '') {
            if ($session->get_local('shelf_no') == '') {
                return;     // 指定が無い場合は何もしない
            } else {
                // リクエストがなくてページ復元以外ならセッションをクリアー
                if ($request->get('page_keep') == '') $session->add_local('shelf_no', '');
            }
        } else {
            // 全角入力防止
            $request->add('shelf_no', mb_convert_kana($request->get('shelf_no'), 'a'));
            $session->add_local('shelf_no', $request->get('shelf_no'));
        }
        // エラーチェックが必要なリクエストはここに記述
    }
    ...
    private function InitTargetMark($request, $session)
    {
        if ($request->get('mark') == '') {
            ...
            ///// AjaxのGETメソッドは SJIS → UTF-8  POSTメソッドは UTF-8 → UTF-8 へ変換
            $session->add_local('mark', mb_convert_encoding($request->get('mark'), 'UTF-8', 'auto'));
        }
        // エラーチェックが必要なリクエストはここに記述
    }
    
    ///// 形状コードの取得・初期化
    private function InitTargetShape_code($request, $session)
    {
        if ($request->get('shape_code') == '') {
            if ($session->get_local('shape_code') == '') {
                return;     // 指定が無い場合は何もしない
            } else {
                // リクエストがなくてページ復元以外ならセッションをクリアー
                if ($request->get('page_keep') == '') $session->add_local('shape_code', '');
            }
        } else {
            // 全角入力防止 <select>で入力しているためコメントアウト
            // $request->add('shape_code', mb_convert_kana($request->get('shape_code'), 'a'));
            $session->add_local('shape_code', $request->get('shape_code'));
        }
        // エラーチェックが必要なリクエストはここに記述
    }
    ...
    private function InitTargetNote_parts($request, $session)
    {
        if ($request->get('note_parts') == '') {
            ...
            ///// AjaxのGETメソッドは SJIS → UTF-8  POSTメソッドは UTF-8 → UTF-8 へ変換
            $session->add_local('note_parts', mb_convert_encoding($request->get('note_parts'), 'UTF-8', 'auto'));
        }
        // エラーチェックが必要なリクエストはここに記述
    }
    ...
    private function InitTargetNote_mark($request, $session)
    {
        if ($request->get('note_mark') == '') {
            ...
            ///// AjaxのGETメソッドは SJIS → UTF-8  POSTメソッドは UTF-8 → UTF-8 へ変換
            $session->add_local('note_mark', mb_convert_encoding($request->get('note_mark'), 'UTF-8', 'auto'));
        }
        // エラーチェックが必要なリクエストはここに記述
    }
    ...
    private function InitTargetNote_shape($request, $session)
    {
        if ($request->get('note_shape') == '') {
            ...
            ///// AjaxのGETメソッドは SJIS → UTF-8  POSTメソッドは UTF-8 → UTF-8 へ変換
            $session->add_local('note_shape', mb_convert_encoding($request->get('note_shape'), 'UTF-8', 'auto'));
        }
        // エラーチェックが必要なリクエストはここに記述
    }
    ...
    private function InitTargetNote_size($request, $session)
    {
        if ($request->get('note_size') == '') {
            ...
            ///// AjaxのGETメソッドは SJIS → UTF-8  POSTメソッドは UTF-8 → UTF-8 へ変換
            $session->add_local('note_size', mb_convert_encoding($request->get('note_size'), 'UTF-8', 'auto'));
        }
        // エラーチェックが必要なリクエストはここに記述
    }
    ...
    {
        require_once ('punchMark_search_ViewCondForm.php');
        return true;
    }
    ...
    private function ViewListExecute($menu, $session, $model, $request, $uniq)
    {
        ...
        case 'List':
            require_once ('punchMark_search_ViewListAjax.php');
            break;
        case 'ListWin':
        default:
            require_once ('punchMark_search_ViewListWin.php');
        }
        return true;
    }
    
} // class PunchMarkSearch_Controller End
