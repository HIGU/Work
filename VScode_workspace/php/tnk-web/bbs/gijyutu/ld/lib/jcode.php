...
// Changed history                                                          //
// 2005/07/15 Created  jcode.php                                            //
// 2006/08/30 ソースを見やすいようにフォーマット                            //
//////////////////////////////////////////////////////////////////////////////
...
function SJIStoJIS(&$str_SJIS)
{
    ...
    $b = unpack('C*', $str_SJIS);
    $n = count($b);
    $ESC = array(chr(0x1B).chr(0x28).chr(0x42), chr(0x1B).chr(0x24).chr(0x42), chr(0x1B).chr(0x28).chr(0x49));
    for ($i=1; $i<=$n; ++$i) {
        $b1 = $b[$i];
        if (0xA1 <=$b1 && $b1 <= 0xDF) {
            if ($mode != 2) {
...
function EUCtoUTF8(&$str_EUC,&$table_jis_utf8)
{
    $str_UTF8 = '';
    $b = unpack('C*', $str_EUC);
    $n = count($b);
...
{
    $table_han2zen_euc = array(
        0xA1A3,0xA1D6,0xA1D7,0xA1A2,0xA1A6,0xA5F2,
    ...
    for ($i=1; $i<=$n; ++$i) {
        ...
        if ($b1 == 0x8E) {
            $b2 = $b[++$i];
            $ofs = 0;
            if ((($b2 == 0xB3) || (0xB6 <= $b2 && $b2 <= 0xC4) || (0xCA <=$b2 && $b2 <=0xCE))&& (isset($b[$i+1])&& $b[$i+1] ==0x8E)) {
                if ($b[$i+2] == 0xDE) {
...
function HANtoZEN_SJIS(&$str_HAN)
{
    ...
    for ($i=1; $i<=$n; ++$i) {
        ...
        if (0xA1 <= $b1 && $b1 <=0xDF) {
            $ofs = 0;
            if ((($b1 ==0xB3) || (0xB6 <= $b1 && $b1 <= 0xC4) || (0xCA <= $b1 && $b1 <= 0xCE)) && isset($b[$i+1])) {
                if ($b[$i+1] == 0xDE) {
...
{
    $table_han2zen_jis = array(
        0x2123,0x2156,0x2157,0x2122,0x2126,0x2572,
    ...
    for ($i=1; $i<=$n; ++$i) {
        ...
        if ($mode == 2) {
            ...
            $b1 = $b[$i];
            $ofs = 0;
            if ((($b1 == 0x33) || (0x36 <= $b1 && $b1 <= 0x44) || (0x4A <= $b1 && $b1 <= 0x4E)) && isset($b[$i+1])) {
                if ($b[$i+1] == 0x5E) {
...
{
    $table_han2zen_utf8_1 = array(
        0xE38082,0xE3808C,0xE3808D,0xE38081,0xE383BB,
    ...
    $table_han2zen_utf8_2 = array(
        0xE382BF,0xE38381,0xE38384,0xE38386,0xE38388,
...
function ZENtoHAN_SJIS(&$str_ZEN, $kana =1, $alph =1, $sym =1)
{
    ...
    for ($i=1; $i<=$n; ++$i) {
        ...
        $b2 = $b[++$i];
        if ($kana == 1 && $b1 == 0x83 && (0x3F < $b2 && $b2 < 0x9F)) {
            $c = $b2 - 0x40;
        ...
        if ($sym == 1 && $b1 == 0x81 && (0x3F < $b2 && $b2 < 0x9F)) {
            $c1 = $sym_sjis[ $b2 - 0x40 ];
            if ($c1 == 0x00) {
                $str_HAN .= chr($b1).chr($b2);
            } else {
        ...
        if ($alph == 1 && $b1 == 0x82 && (0x3F < $b2 && $b2 < 0x9F)) {
            if ($b2 < 0x80) {
                $str_HAN .= chr($b2 - 0x1F);
            } else {
                $str_HAN .= chr($b2 - 0x20);
...
function jsubstr($str, $start =0, $length =0)
{
    ...
    if (!isset($jstr)) $jstr[] =''; $n = count($jstr);
    if ($start < 0) $start += $n;
