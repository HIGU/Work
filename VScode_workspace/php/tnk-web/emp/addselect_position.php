...
if(addselectPosition($_POST['pid'],$_POST['position_name'],$_POST['pflg'])){
    header('Location: ' . H_WEB_HOST . EMP . 'emp_menu.php?func=' . FUNC_CHGINDICATE . '#position');
    exit();
}
$_SESSION['s_sysmsg'] .= '<br>職位の項目追加に失敗しました。';  // addselectPositionでエラーの場合にメッセージがあるため.=に変更
header('Location: ' . H_WEB_HOST . EMP . 'emp_menu.php?func=' . FUNC_CHGINDICATE . '#position');
?>
