<?php
//////////////////////////////////////////////////////////////////////////////
// 組立の実績工数と登録工数の比較 照会                Client interface 部   //
//                                              MVC Controller の Main 部   //
// Copyright (C) 2006-2006 Kazuhiro.Kobayashi tnksys@nitto-kohki.co.jp      //
// Changed history                                                          //
// 2006/03/01 Created   assembly_time_show_Main.php                         //
// 2006/03/07 タイトル名称を変更(実績工数を最初に)                          //
//////////////////////////////////////////////////////////////////////////////
...
require_once ('assembly_time_show_Controller.php'); // MVC の Controller部
require_once ('assembly_time_show_Model.php');      // MVC の Model部
...
function main()
{
    ...
    ////////////// サイト設定
    $menu->set_site(INDEX_INDUST, 8);           // site_index=INDEX_INDUST(生産メニュー) site_id=8(組立工数照会)999(サイトを開く)
    ////////////// リターンアドレス設定
    // $menu->set_RetUrl(EQUIP_MENU);              // 通常は指定する必要はない
    
    //////////// タイトル名(ソースのタイトル名とフォームのタイトル名)
    $title = '組立の実績工数と登録工数の比較 照会';
    $menu->set_title($title);
    //////////// 呼出先のaction名とアドレス設定
    // $menu->set_action('引当構成表',   INDUST . 'material/allo_conf_parts_view.php');
    
    //////////// コントローラー部のインスタンス生成
    $controller = new AssemblyTimeShow_Controller($menu);
