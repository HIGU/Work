<?php
//////////////////////////////////////////////////////////////////////////////
// 新版 製品グループコード編集 メイン product_groupMaster_Main2.php         //
// 製品グループ（詳細）の検索用グループ設定                                 //
// Copyright (C) 2011 Norihisa.Ohya norihisa_ooya@nitto-kohki.co.jp         //
// Changed history                                                          //
// 2011/05/31 Created  product_groupMaster_Main2.php                        //
//////////////////////////////////////////////////////////////////////////////
...
{       
    ...
    ////////// Viewの処理
    require_once ('product_groupMaster_View2.php');
...
function machineWork_entry($request, $result)
{
    if (getCheckAuthority(22)) {                             // 認証チェック
        ...
        $query = sprintf("SELECT mhgcd FROM msshg3 WHERE mhgcd='%s'", $mhgcd);
        $res_chk = array();
        if ( getResult($query, $res_chk) > 0 ) {         // 登録あり UPDATE 更新
            $query = sprintf("UPDATE msshg3 SET mhggp=%d, last_date=CURRENT_TIMESTAMP, last_user='%s' 
...
function getViewHTMLbody($request, $menu, $result)
{
    ...
    $listTable .= "<body>\n";
    $listTable .= "<center>\n";
    $listTable .= "    <form name='entry_form' action='product_groupMaster_Main2.php' method='post'>\n";
    ...
    for ($r=0; $r<$result->get('rows_m'); $r++) {
        ...
        $listTable .= "                <a href='../product_groupMaster_Main2.php?number=". $r ."' target='_parent' style='text-decoration:none;'>\n";
        $cnum = $r + 1;
        $listTable .= "                ". $cnum ."\n";
        $listTable .= "                </a>\n";
