...
// Changed history                                                          //
// 2005/09/12 Created   parts_pickup_linear_Controller.php                  //
...
class PartsPickupLinear_Controller
{
    ///// Private properties
    ...
    ************************************************************************/
    ...
    public function display($menu, $request, $result, $model)
    {
        ...
        case 'EndList':                                     // 出庫完了 一覧表 表示
            ...
            require_once ('parts_pickup_linear_ViewEndList.php');
            break;
        case 'apend':                                       // 出庫着手の入力 (追加) 複数の計画対応のため ApendListを取得する
            ...
            if ($user_id == '') {
                require_once ('parts_pickup_linear_ViewApendUserID.php');
            } else {
                require_once ('parts_pickup_linear_ViewApend.php');
            }
            break;
        case 'user':                                        // 出庫 作業者 登録 一覧表 表示
            ...
            require_once ('parts_pickup_linear_ViewUser.php');
            break;
        case 'TimeEdit':                                    // 着手・完了の時間 修正 画面 表示
            ...
            $pageParm = $model->get_htmlGETparm() ."&id={$uniq}";
            require_once ('parts_pickup_linear_TimeEdit.php');
            break;
        ...
        default:                // リクエストデータにエラーの場合は初期値の着手一覧を表示
            ...
            $pageControl = $model->out_pageCtlOpt_HTML($menu->out_self()."?id={$uniq}");
            require_once ('parts_pickup_linear_ViewList.php');
            break;
        }
...
} // Class PartsPickupLinear_Controller End
