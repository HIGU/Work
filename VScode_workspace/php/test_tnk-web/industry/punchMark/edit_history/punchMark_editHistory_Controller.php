<?php
//////////////////////////////////////////////////////////////////////////////
// 刻印管理システム 編集履歴メニュー                     MVC Controller 部  //
// Copyright (C) 2007      Kazuhiro.Kobayashi tnksys@nitto-kohki.co.jp      //
// Changed history                                                          //
// 2007/11/15 Created   punchMark_editHistory_Controller.php                //
//////////////////////////////////////////////////////////////////////////////
...
class PunchMarkEditHistory_Controller
{
    ///// Private properties
    ...
    ************************************************************************/
    ...
    public function __construct($menu)
    {
        ...
        $this->model = new PunchMarkEditHistory_Model();
    }
    ...
    ***************************************************************************/
    ...
    protected function Init($request, $session)
    {
        ...
        // 対象マスターの処理
        $this->InitTargetMaster($request, $session);
        // 対象更新内容の処理
    ...
    ***************************************************************************/
    ...
    ///// 対象マスターの取得・初期化
    private function InitTargetMaster($request, $session)
    {
        if ($request->get('targetMaster') == '') {
            if ($session->get_local('targetMaster') == '') {
                return;     // 指定が無い場合は何もしない
            } else {
                // リクエストがなくてページ復元以外ならセッションをクリアー
                if ($request->get('page_keep') == '') $session->add_local('targetMaster', '');
            }
        } else {
            // 全角入力防止
            // $request->add('targetMaster', mb_convert_kana($request->get('targetMaster'), 'a'));
            $session->add_local('targetMaster', $request->get('targetMaster'));
        }
    ...
    {
        require_once ('punchMark_editHistory_ViewCondForm.php');
        return true;
    }
    ...
    private function ViewListExecute($menu, $session, $model, $request, $uniq)
    {
        ...
        case 'List':
            require_once ('punchMark_editHistory_ViewListAjax.php');
            break;
        case 'ListWin':
        default:
            require_once ('punchMark_editHistory_ViewListWin.php');
        }
        return true;
    }
    
} // class PunchMarkEditHistory_Controller End
