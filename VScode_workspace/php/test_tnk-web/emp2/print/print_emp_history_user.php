...
// Changed history                                                          //
// 2004/02/16 Created  print_emp_history_mbfpdf.php → _history_user.php へ //
// 2004/03/01 fpdf_protection.php を使い 印刷のみのプロテクトをかけた。     //
// 2005/10/13 住所を塩谷郡氏家町→さくら市へ変更 fullpage→fullwidth へ変更 //
// 2007/03/07 移動を異動へ訂正                                              //
// 2007/10/15 メモリーリミットを追加。 E_ALL → E_ALL | E_STRICTへ          //
//            ファイル名をprint_emp_history_user.phpへ変更し個人別出力対応  //
// 2007/10/16 作成・承認欄を削除して住所・電話番号欄へ キャプションを青色へ //
// 2010/06/16 暫定的に大渕さん（970268）が印刷できるように変更         大谷 //
// 2017/09/13 社長・顧問・その他・日東工器所属を除外                   大谷 //
//////////////////////////////////////////////////////////////////////////////
ini_set('memory_limit', '64M');             // PDFの大量出力のため 52MでOKだが 64Mへ
...
class PDF_j extends MBFPDF  // 日本語PDFクラスを拡張します。
{
    ...
    function Header()   //頭につけたい内容はここに。コンストラクタみたいに自動実行されます。
    {
        ...
        **********/
        ...
        $this->Cell(20, 7, '部　署', 'LRTB', 0, 'C', 1);
        $this->Cell(30, 7, $this->data_usr[1], 'LRTB', 0, 'C', 1);
...
if (isset($_REQUEST['targetUser'])) {
    $where_user = "uid = '{$_REQUEST['targetUser']}' AND ";
} else {
    $where_user = '';
}
///// $where_user = "uid = '{$_SESSION['User_ID']}' AND ";
$sql = "SELECT trim(uid) AS uid
        ...
        LEFT OUTER JOIN
            position_master
        USING(pid)
        WHERE {$where_user}sflg = 1 AND retire_date IS null AND uid != '000000' and pid != 120 and sid != 80 and sid != 90 and sid != 95
...
while ($row = pg_fetch_object($res_usr)) {
    ...
    $now_section  = mb_substr($row->section, -6);       // 部署(短縮)
    $now_position = mb_substr($row->position, 0, 3);    // 職位(短縮)
