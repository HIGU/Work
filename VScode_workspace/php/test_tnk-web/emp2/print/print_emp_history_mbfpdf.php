...
// Changed history                                                          //
// 2004/02/16 Created  print_emp_history_mbfpdf.php  ゴシック体             //
...
// 2007/10/16 教育・資格・移動(キャプション)を青色へ  data_usr初期化を変更  //
// 2008/04/24 部署名と役職の短縮文字数を変更                           大谷 //
// 2010/06/16 暫定的に大渕さん（970268）が印刷できるように変更         大谷 //
// 2010/06/17 社員数増加によりメモリーリミットを64Mから100Mに変更      大谷 //
// 2017/09/13 社長・顧問・その他・日東工器所属を除外                   大谷 //
// 2018/04/20 押印欄を社員毎の先頭ページのみ印刷するよう変更           大谷 //
// 2019/07/22 印刷許可をgetCheckAuthority(59)に、押印を自動に変更      大谷 //
//            社員番号.jpgなので変更の場合は画像作成(Excel)とPGM変更を 大谷 //
// 2020/05/18 押印を川・害歡溝緲・吠儿・                                大谷 //
//////////////////////////////////////////////////////////////////////////////
...
class PDF_j extends MBFPDF  // 日本語PDFクラスを拡張します。
{
    ...
    function Header()   //頭につけたい内容はここに。コンストラクタみたいに自動実行されます。
    {
        ...
        if ($this->usr_cnt == 1) {  // 押印欄表示用 先頭のみ押印表示
            ...
            $this->Image('/var/www/html/emp/print/300055.jpg', 170, 28, 12, 12, '', '');
            $this->Cell(20, 15, '', 'LRTB', 0, 'C');
            $this->Image('/var/www/html/emp/print/300551.jpg', 190, 28, 12, 12, '', '');
            $this->SetY(42);
            $this->usr_cnt = 0;
        } else {
            $this->Cell(20,  5, '', '', 0, 'C');
