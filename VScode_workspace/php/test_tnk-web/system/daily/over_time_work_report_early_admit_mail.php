...
<?php
//////////////////////////////////////////////////////////////////////////////
// 定時間外作業申告【早出】承認待ち情報メール送信 cron.d tnk_daily 処理で実行//
// 月～金の指定時にメール送信 全体版 【早出】承認待ちがある全承認者に定期メール//
...
fwrite($fpb, "定時間外作業申告【早出】承認待ち情報メール送信\n");
...
if ($con = db_connect()) {
    ...
    echo "$log_date 定時間外作業申告【早出】承認待ち情報メール送信 db_connect() error \n";
    fwrite($fpa,"$log_date 定時間外作業申告【早出】承認待ち情報メール送信 db_connect() error \n");
    fwrite($fpb,"$log_date 定時間外作業申告【早出】承認待ち情報メール送信 db_connect() error \n");
    exit();
}
...
//    if( $superiors ) $sendna = $res_list[$n][3];    // 名前 強制変更 ※リリース時は、コメント化
    if( $superiors ) {
        $attenSubject = "{$sendna} 様 【不在未承認】 定時間外作業申告【早出】よりお知らせ"; // 宛先： 
    } else {
        $attenSubject = "{$sendna} 様 【未承認】 定時間外作業申告【早出】よりお知らせ";
    }
    $message = "{$sendna} 様\n\n";
    if( $superiors ) {
        $message .= "{$res_list[$r][3]} 様 不在の為、代わりに\n\n";
        $message .= "定時間外作業申告【早出】（事前申請）承認処理をお願いします。\n\n";
