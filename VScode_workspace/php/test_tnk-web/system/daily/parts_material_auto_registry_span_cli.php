...
//            期間指定で登録する(登録できなかったものの対応)                //
// 2006/03/02 2/28のCQ36860-0で当日単価登録で当日計上への対応(未満を以下へ) //
//      as_regdate<{$res[$i]['計上日']} → as_regdate<={$res[$i]['計上日']} //
// 2007/08/08 売上計上日より前の登録を使用→以前へ変更。それでもない場合は  //
...
$fpa = fopen($log_name_a, 'a+');    // 全てのログ w=過去のログを消す a=追加
...
for ($i=0; $i<$rows; $i++) {
    ...
    if (getResultTrs($con, $query, $resCost) <= 0) {    // トランザクション内での 照会専用クエリー
        $log_date = date('Y-m-d H:i:s');    // 日報用ログの日時
        fwrite($fpa, "{$log_date} {$res[$i]['部品番号']} は計上日 {$res[$i]['計上日']} 前に単価登録が無い {$res[$i]['部品名']}\n");
        /////////// トランザクションのロールバック
        query_affected_trans($con, 'ROLLBACK');
        $rec_ng++;
        continue;
