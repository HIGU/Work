...
fwrite($fpb, "/var/www/html/system/daily/daily_backet_ftp_cli_nkb.php\n");
echo "/var/www/html/system/daily/sales_get_ftp_nkb.php\n";
...
if (file_exists($file_name)) {            // ファイルの存在チェック
    ...
    while (FALSE !== ($data = fgetcsv($fp, 300, ',')) ) {    // CSV file として読込み
        ...
        $data[2] = mb_convert_encoding($data[2], 'SJIS', 'UTF-8');   // EUC-JPをSJISへ変換
...
if (file_exists($file_name)) {            // ファイルの存在チェック
    ...
    while (FALSE !== ($data = fgetcsv($fp, 300, ',')) ) {    // CSV file として読込み
        ...
        if (($rows_csv = getResultWithField3($query_csv, $field_csv, $res)) <= 0) {
            ...
            $res_csv[0][1] = "";
            $res_csv[0][2] = "";
            $res_csv[0][3] = mb_convert_encoding($res[0][1], 'SJIS', 'UTF-8');   // EUC-JPをSJISへ変換
            $res_csv[0][4] = $data[1];
