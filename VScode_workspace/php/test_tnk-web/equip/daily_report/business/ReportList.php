...
// 2004/09/27 工場別運転日報に対応 セッション変数による工場区分対応         //
// 2005/02/02 確定の取消ボタンを追加 FNC_ACCOUNT のユーザーのみ実行できる   //
// 2005/02/25 MakeSql()のsub_queryを別にしFromDateとToDate条件を追加(高速化)//
// 2007/04/04 phpのショートカットタグを標準タグへ変更(推奨タグへ変更)。     //
//            widthを機械名(65)と部品名(150)に追加。                        //
//            EntryCheck()にmu_Date対策の(月・日)を２桁にするロジック追加   //
// 2007/04/11 手入力対応で MakeSql() のSQL文を変更                          //
// 2021/03/11 備考の検索を追加                                         大谷 //
//////////////////////////////////////////////////////////////////////////////
...
if (isset($_REQUEST['Undecision'])) {
    ...
    $query = "UPDATE equip_work_report SET decision_flg=0 where work_date={$work_date} and mac_no={$mac_no} and siji_no={$siji_no} and koutei={$koutei}";
    if (query_affected($query) > 0) {
        $_SESSION['s_sysmsg'] = "確定を取消ました。  運転日={$work_date}  機械No.={$mac_no}  指示No.={$siji_no}  工程No.={$koutei}";
    } else {
        $_SESSION['s_sysmsg'] = "確定の取消が出来ませんでした！  運転日={$work_date}  機械No.={$mac_no}  指示No.={$siji_no}  工程No.={$koutei}  管理担当者に連絡して下さい。";
    }
}
...
function MakeSql()
{
    ...
    $ToDate    = $Parameter['ToYear'] . $Parameter['ToMonth'] . $Parameter['ToDay'];
    $sub_query = "(
                    select work_date, mac_no, siji_no, koutei
                        , min(to_char(from_date,'99999999') || to_char(from_time,'9999')) as datetime
                    from
                        equip_work_report_log
                    where
                        work_date>={$FromDate} and work_date<={$ToDate} -- 2005/02/25 ADD
                    group by
                        work_date, mac_no, siji_no, koutei
                )
    ";
    $sub_query2 = "(
                    select work_date, mac_no, siji_no, koutei, mac_state
                        , min(to_char(from_date,'99999999') || to_char(from_time,'9999')) as datetime
                    from
                        equip_work_report_log
                    where
                        ...
                        and mac_state= '6'
                    group by
                        work_date, mac_no, siji_no, koutei, mac_state
                )
    ";
    ...
    $sql = " select                     "
         ...
         . " left outer join equip_work_inst_header b on a.siji_no=b.inst_no "
         . " left outer join equip_work_log2_header c on a.mac_no=c.mac_no and a.siji_no=c.siji_no and a.koutei=c.koutei "
         . " left outer join miitem d on d.mipn=b.parts_no "
         . " left outer join equip_machine_master2 e on a.mac_no=e.mac_no "
         . " left outer join {$sub_query} f on f.work_date=a.work_date and f.mac_no = a.mac_no and f.siji_no=a.siji_no and f.koutei=a.koutei "
         . " left outer join {$sub_query2} r on r.work_date=a.work_date and r.mac_no = a.mac_no and r.siji_no=a.siji_no and r.koutei=a.koutei ";
    if ($factory != '') {
    ...
    $sql .= ' order by a.work_date desc, mac_no, datetime desc, siji_no';
...
function doView(Type,WorkDate,SijiNo,MacNo,KouteiNo) {
...
function Undecision(work_date, mac_no, siji_no, koutei) {
    ...
    document.CancelForm.siji_no.value    = siji_no;
...
<input type='hidden' name='siji_no' value=''>
...
<input type='hidden' name='SijiNo' value=''>
...
?>
            <tr>
                <td align='center' nowrap>
                    <input type='button' value='通常' onClick="doView(1,'<?php echo outHtml($row['work_date'])?>','<?php echo outHtml($row['siji_no'])?>','<?php echo outHtml($row['mac_no'])?>','<?php echo outHtml($row['koutei'])?>')"><input type="button" value="集計" onClick="doView(2,'<?php echo outHtml($row['work_date'])?>','<?php echo outHtml($row['siji_no'])?>','<?php echo outHtml($row['mac_no'])?>','<?php echo outHtml($row['koutei'])?>')"><?php if ($AdminUser) { ?><input type="button" value="修正" onClick="doEdit('<?php echo outHtml($row['work_date'])?>','<?php echo outHtml($row['siji_no'])?>','<?php echo outHtml($row['mac_no'])?>','<?php echo outHtml($row['koutei'])?>')"<?php echo $DECISION?>><?php } ?>
                </td>
                <td align="center" nowrap>
                    <?php echo outHtml(mu_Date::toString($row['work_date'],"Y/m/d"))?>
                </td>
                <td align="center" nowrap>
                    <?php echo outHtml($row['siji_no'])?>
                </td>
                <td align="center" nowrap>
                ...
                <td nowrap align='center'>
                    <?php if ($suUser) { ?>
                    <input type='button' value='取消' onClick='Undecision(<?php echo $row['work_date']?>, <?php echo $row['mac_no']?>, <?php echo $row['siji_no']?>, <?php echo $row['koutei']?>)'>
                    <?php } else { ?>
