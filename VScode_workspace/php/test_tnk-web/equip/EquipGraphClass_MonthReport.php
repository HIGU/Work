...
// 2005/09/30 日報用を雛型にした月次グラフ EquipGraphClass_MonthReport.php  //
//////////////////////////////////////////////////////////////////////////////
...
define('EGMR_VERSION', '1.00'); // EGR_VERSION 1.04ベース
...
class EquipGraphReport
{
    ...
    ************************************************************************/
    ...
    function EquipGraphReport($mac_no, $str_date, $end_date, $sampling=180)
    {
        if (!isset($_SESSION)) {                    // セッションの開始チェック
            session_start();                        // Notice を避ける場合は頭に@
        }
        if ($this->set_condition($str_date, $end_date, $mac_no) == false) {
            $addr_status = "ErrorEquipGraphPage.php?status=2&date=$str_date&mac_no=$mac_no";
            header('Location: ' . H_WEB_HOST . ERROR . $addr_status);
            exit();
        }
        $this->set_xtimeArrVal();               // X軸のセットできる配列の初期化
        $this->xtime = 744;                     // 初期化 Default=744hr 31日(月報ベース)
    ...
    function set_condition($str_date, $end_date, $mac_no)
    {
        if ($mac_no  == '') return false;
        $query = "
            select
                to_char(CAST('{$str_date} 08:30:00' AS TIMESTAMP), 'YYYY/MM/DD HH24:MI:SS') AS str_timestamp
                , CASE
                    WHEN (CAST('{$end_date} 08:30:00' AS TIMESTAMP) + interval '24 hour') > CURRENT_TIMESTAMP THEN
                        to_char(CURRENT_TIMESTAMP, 'YYYY/MM/DD HH24:MI:SS')
                    ELSE
                        to_char(CAST('{$end_date} 08:30:00' AS TIMESTAMP) + interval '24 hour', 'YYYY/MM/DD HH24:MI:SS')
                  END
                AS end_timestamp
        ";
        $res = array();
    ...
    // グラフのタイトル名を設定
    function set_title($title='月次 稼動 時間 グラフ')
    {
        //////////////// 機械マスターから機械名を取得
    ...
    function out_graph($graph_name='', $summary='no')
    {
        ...
        $graph->xaxis->title->Set(mb_convert_encoding('日数', 'UTF-8'));
    ...
    ***************************************************************************/
    ...
    {
        $this->xtimeArrVal ['1日間'] = 24;
        $this->xtimeArrVal ['2日間'] = 48;
        $this->xtimeArrVal ['3日間'] = 72;
        $this->xtimeArrVal ['4日間'] = 96;
        $this->xtimeArrVal ['5日間'] = 120;
        $this->xtimeArrVal ['7日間'] = 168;
        $this->xtimeArrVal ['10日間'] = 240;
        $this->xtimeArrVal ['15日間'] = 360;
        $this->xtimeArrVal ['27日間'] = 648;
        $this->xtimeArrVal ['28日間'] = 672;
        $this->xtimeArrVal ['29日間'] = 696;
        $this->xtimeArrVal ['30日間'] = 720;
        $this->xtimeArrVal ['31日間'] = 744;
        /***    これは日報の場合
