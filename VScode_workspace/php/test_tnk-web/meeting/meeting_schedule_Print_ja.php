<?php
///////////////////////////////////////////////////////////////////////////////
// 全社共有 会議スケジュール スケジュールＰＤＦ出力(印刷) FPDF/FPDF-JA使用   //
// Copyright (C) 2009 Norihisa.Ohya norihisa_ooya@nitto-kohki.co.jp          //
// Changed history                                                           //
// 2009/12/21 Created   meeting_schedule_Print_ja.php                        //
///////////////////////////////////////////////////////////////////////////////
...
$data_temp = array();
$data_temp = explode( '&', $_SERVER["QUERY_STRING"]);
$room_temp = explode( 'room_no=', $data_temp[0]);
$str_temp  = explode( 'str_date=', $data_temp[1]);
$end_temp  = explode( 'end_date=', $data_temp[2]);
$room_no   = $room_temp[1];
$str_date  = $str_temp[1];
$end_date  = $end_temp[1];
...
class PDF_j extends PDF_Japanese                           // 日本語PDFクラスを拡張します。
{
    ...
    {   
        ...
        $this->SetY(251);
        $this->SetX(18);
        $header = array('', '工場長', '副工場長', '品質管理責任者', '管理部長', '品証課長');
...
if ($room_no != '') {
    $where = "WHERE room_no = {$room_no} and to_char(str_time, 'YYYYMMDD') >= {$str_date} and to_char(end_time, 'YYYYMMDD') <= {$end_date}";
} else {
    $where = "WHERE to_char(str_time, 'YYYYMMDD') >= {$str_date} and to_char(end_time, 'YYYYMMDD') <= {$end_date}";
}
///// 会議スケジュール全社共有ヘッダーテーブル の取得
$sql = "    SELECT serial_no                            -- serial_no
                ,subject                                -- subject
                ,to_char(str_time, 'YY/MM/DD HH24:MI')  -- str_ym
                ,to_char(end_time, 'YY/MM/DD HH24:MI')  -- end_ym
                ,room_name                              -- room_name
                ,sponsor                                -- 05
                ,trim(name)             AS 氏名         -- sponsor
                ,atten_num                              -- 07
                ,CASE
...
$data_header = array();
$data_header = array($row->serial_no, $row->subject, $row->str_ym, $row->end_ym, $row->room_name, $row->sponsor);

///// 会議スケジュール全社共有参加者テーブル の取得
$rowsAtten = array(); $resAtten = array();      // 初期化
$data_atten = array();
for ($i=0; $i<count($data_header); $i++) {
    $rowsAtten[$i] = getViewAttenList($data_header[$i][0]);
    $resAtten[$i]  = $data_atten;
}
function getViewAttenList($serial_no)
{
    $sql = "    SELECT serial_no                            -- 00
                    ,atten                                  -- 01
                    ,trim(name)                             -- atten_name
                    ,CASE
                        WHEN mail THEN '送信済'
                        ELSE '未送信'
                     END                                    -- 03
    ...
    // スカラー変数ではなくて、配列だということを明示
    $data_atten = array($row->atten_name);
    return $row;
}
