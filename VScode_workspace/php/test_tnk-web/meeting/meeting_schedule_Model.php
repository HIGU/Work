    ...
    public function __construct($request)
    {
       ...
       case 'Car':
        ...
        case 'Absence'  :
            ...
            break;
        case 'Over'  :
            $this->where = "WHERE date='{$year}-{$month}-{$day}' AND yo_ad_rt IS NOT NULL AND yo_ad_rt!='-1'";
            $sql_sum = "
                SELECT count(*) FROM over_time_report {$this->where}
            ";
            break;
        case 'List'  :
        case 'Apend' :
    ...
    public function add($request)
    {
        ...
        if ($this->duplicateCheck("{$year}-{$month}-{$day} {$str_time}:00", "{$year}-{$month}-{$day} {$end_time}:00", $room_no)) {
            if ($this->cardupCheck("{$year}-{$month}-{$day} {$str_time}:00", "{$year}-{$month}-{$day} {$end_time}:00", $car_no)) {
                ...
                if ($serial_no) {
//                    if ($room_no == 2200 && $sponsor == 300667) { // ※リリース時 sponsor 以降削除
                    if ($room_no == 2200) {
                        $this->outingMail($request);
                    }
                    if ($mail == 't') {
...
//                            $this->outingMail($request);
    ...
    public function getViewAttenList(&$result, $serial_no)
    {
        ...
        ";
        $res = array(
            [1, ["test1", "test2", "test3",], "dummy1", '送信済'],
            [2, ["test1", "test2", "test3",], "dummy2", '送信済'],
            [3, ["test1", "test2", "test3",], "dummy3", '送信済'],
            [4, ["test1", "test2", "test3",], "dummy4", '送信済'],
            [5, ["test1", "test2", "test3",], "dummy5", '送信済']
        );
        $rows = count($res);
        $result->add_array($res);
        return $rows;
    ...
    ***************************************************************************/
    ...
    ////////// 外出するので管理部長へ email を出す
    protected function outingMail($request, $cancel=false)
    {
        ///// パラメーターの分割
        $year       = $request->get('yearReg');             // 会議予定の年４桁
        $month      = $request->get('monthReg');            // 会議予定の月２桁
        ...
        // 主催者の名前を取得
        $sponsor_name = $this->getName($sponsor);
        // 会議室名の取得
        $room_name = $this->getRoomName($room_no);
        // 出席者の名前取得 (引数３個は全て配列)
        $this->getAttendanceName($atten, $atten_name, $flag);
        // 承認者のメールアドレスの取得
        $to_uid = $this->GetKanriButyoUid();
        if( $this->IsAbsence($to_uid) ) {   // 不在
            $to_uid = $this->GetSoumuKatyoUid();
        }
        $to_name = $this->getName( $to_uid );
        $to_addres = $this->getMailAddres( $to_uid );
        // 承認者へメール送信
        $attenSubject = "【外出情報】 $to_name 様　確認下さい。";
        
        $message  = "この案内は、{$sponsor_name} さんが外出を登録したため送信されたものです。\n";
        $message .= "下記の日時で外出しますので、承認お願い致します。\n\n";
        $message .= "                               記\n\n";
        $message .= "１. 日　付：{$year}年 {$month}月 {$day}日({$dayWeek})\n";
        $message .= "２. 時　間：{$str_time} ～ {$end_time}\n";
        $message .= "３. 場　所：{$room_name}\n";
        $message .= "４. 主催者：{$sponsor_name}\n";
        $message .= "５. 出席者：{$this->getAttendanceNameList($atten, $atten_name)}\n";
        $message .= "６. 件　名：{$subject2}\n\n";
        $message .= "http://10.1.3.252/meeting/meeting_schedule_Main.php?calUid={$to_uid}&year={$year}&month={$month}&day={$day}\n\n";
        $message .= "以上、宜しくお願い致します。\n\n";
        $add_head = "";
        if (mb_send_mail($to_addres, $attenSubject, $message, $add_head)) {
            // 出席者へのメール送信履歴を保存
...
//                }
    ...
    ***************************************************************************/
    ...
    // 指定UIDのメールアドレス取得
    public function getMailAddres($uid)
    {
//        $uid = "300667"; // 強制変更 ※リリース時は、コメント化
        $query = "SELECT trim(mailaddr) FROM user_detailes LEFT OUTER JOIN user_master USING(uid) WHERE uid='$uid'";
        $res = array();
        if ($this->getResult2($query, $res) < 1) {
            return "";
        }
        return $res[0][0]; // メールアドレス
    }

    ////////// 外出の承認
    public function oadmit($request)
    {
        ///// パラメーターの分割
        $serial = $request->get('outing_serial');   // 承認するシリアル番号
        echo "SELECT * FROM meeting_schedule_header WHERE serial_no=$serial";
        
        // 外出を外出 承認へ変更
        $update_sql = "UPDATE meeting_schedule_header SET room_no=2222 WHERE serial_no=$serial";
    ...
    ////////// 残業予定の取得
    public function getViewOver(&$result)
    {
        $query = sprintf( "
            SELECT yo_ad_rt, yo_ad_st, sm.section_name, ud.name, yo_str_h, yo_str_m, yo_end_h, yo_end_m, yo_content
            FROM                over_time_report
            LEFT OUTER JOIN     user_detailes   AS ud    USING(uid)
            LEFT OUTER JOIN     cd_table        AS ct    USING(uid)
            LEFT OUTER JOIN     section_master  AS sm    USING(sid)
            LEFT OUTER JOIN     act_table       AS at    USING(act_id)
            {$this->where}
            ORDER BY ct.orga_id ASC, yo_str_h ASC, yo_str_m ASC, yo_end_h ASC, yo_end_m ASC
        ");
        $res = array();
        if( ($rows=$this->getResult2($query, $res)) < 1 ) {
            return false;
    ...
    ////////// 管理部長ですか？
    public function IsKanriButyo($uid)
    {
        if( ! $uid ) return false;
        
        $query = sprintf( "
            SELECT ud.uid, sm.section_name, pm.position_name, ud.pid, ud.sid
    ...
    ////////// 総務課課長ですか？
    public function IsSoumuKatyo($uid)
    {
        if( ! $uid ) return false;
    ...
    ////////// 管理部長のUIDを取得
    public function GetKanriButyoUid()
    {
        $query = sprintf( "
            SELECT ud.uid
            FROM   user_detailes ud, section_master sm, position_master pm
    ...
    ////////// 総務課長のUIDを取得
    public function GetSoumuKatyoUid()
    {
        $query = sprintf( "
            SELECT ud.uid
            FROM   user_detailes ud, section_master sm, position_master pm
    ...
    ////////// 不在ですか？
    public function IsAbsence($uid)
    {
        $query = sprintf( "
            SELECT uid
            FROM   working_hours_report_data_new
            WHERE  uid='$uid' AND working_date=to_char(CURRENT_DATE,'YYYYMMDD')
