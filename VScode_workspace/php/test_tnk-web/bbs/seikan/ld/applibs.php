...
class View
{
    ...
    function getTemplateName(&$context)
    {
        $cgi = $context->getCgi();
        $tmp = get_class($this);
        $tmp = eregi_replace("view$","",$tmp);
        $agent = $cgi->getAgent();
        if ($agent =="p") {
            return APP_TEMPLATE_DIR.$tmp."Template.php";
        } elseif ($agent =="i") {
            return APP_TEMPLATE_DIR_I.$tmp."Template.php";
        } elseif ($agent =="j") {
            return APP_TEMPLATE_DIR_J.$tmp."Template.php";
        } elseif ($agent =="e") {
            return APP_TEMPLATE_DIR_E.$tmp."Template.php";
        }
...
class Data_Model_Res extends Data_Model
{
    ...
    function cnv_mstring($text)
    {
        ...
        $text = eregi_replace("(https?)(://[[:alnum:]\S\$\+\?\.\;\=\_\%\,\:\@\!\#\~\*\/\&\-]+)","<a href=\"\\1\\2\" target=\"_blank\">\\1\\2</a>",$text);
        return $text;
    }
}
...
class Data_Model_User extends Data_Model
{
    ...
    function cnv_mstring($text)
    {
        if (substr($text,0,3) == "-m-") {
            $text = substr($text,3);
            $text = ereg_replace("\r|\n","",$text);
            $text = ereg_replace("<([^/].*) style([^=]*)=([^>]+)>","<\\1>",$text);
            $text = ereg_replace("<([^/].*) on([^>]+)>","<\\1>",$text);
