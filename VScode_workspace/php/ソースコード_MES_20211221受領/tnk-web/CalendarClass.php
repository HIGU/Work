...
// 2020/11/10 22期の休みに対応(スポーツの日・海の日・体育の日等)       大谷 //
//            何も無ければ特措法は22期までなので元に戻る               大谷 //
// 2020/11/11 今のところ特措法なしなので22期の休みを元に戻した         大谷 //
// 2020/12/16 22期の休みに対応(スポーツの日・海の日・体育の日等)       大谷 //
// 2021/10/26 2023年1月3日が自動的に休業になってしまう為調整           大谷 //
// 2021/10/28 祝祭日の名前を正式名称に（スポーツの日、こどもの日等）   大谷 //
// 2021/11/18 会議帯の23期休(2022年）日を調整4/29出勤、5/2休日へ       大谷 //
//////////////////////////////////////////////////////////////////////////////
    ...
    ***************************************************************************/
    ...
    protected function set_holiday($year, $month)
    {
        ...
        case 5:
            ...
            if ($year == 2022) {
                $this->holiday[2] = 1;
                $this->holiday_name[2] = '振替休日';
            }
            break;
        case 7:
            // 海の日
            // 2020年、2021年は東京オリンピック・パラリンピック特別措置法により
            // 海の日とスポーツの日が固定、何も無ければ2022年には元に戻る
            // 海の日はそのままでスポーツの日は10月の体育の日に
            if ($year > 2002) {
            ...
            if ($year == 2021) {
                ...
                $this->holiday_name[$day+14] = '';
                $this->holiday[22] = 1;
                $this->holiday_name[22] = '海の日';
                $this->holiday[23] = 1;
                $this->holiday_name[23] = 'スポーツの日';
            }
            
            break;
        case 8:
            // 2020年、2021年は東京オリンピック・パラリンピック特別措置法により
            // 山の日が10日と8日に固定、何も無ければ2022年には11日に戻る
            if ($year > 2017) {
                $this->holiday[11] = 1;
        ...
        case 10;
            // 体育の日
            // 2020年、2021年は東京オリンピック・パラリンピック特別措置法により
            // 体育の日が7月のスポーツの日に移動、何も無ければ2022年には元に戻る
            if ($year < 2000) {
                $this->holiday[10] = 1;
