<?php
//////////////////////////////////////////////////////////////////////////////
// 適正在庫数の照会 直近三年間の出荷数÷３×２          Client interface 部 //
...
require_once ('reasonable_stock_Controller.php');       // MVC の Controller部
require_once ('reasonable_stock_Model.php');            // MVC の Model部
...
function main()
{
    ...
    ////////////// サイト設定
    $menu->set_site(INDEX_INDUST, 16);                  // site_index=INDEX_INDUST(生産メニュー) site_id=15(長期滞留部品)999(サイトを開く)
    ////////////// リターンアドレス設定
    // $menu->set_RetUrl(EQUIP_MENU);                   // 通常は指定する必要はない
    
    //////////// タイトル名(ソースのタイトル名とフォームのタイトル名)
    $title = '適正在庫数の照会 在庫基準年月指定';
    $menu->set_title($title);
    //////////// 呼出先のaction名とアドレス設定
    $menu->set_action('在庫経歴',   INDUST . 'parts/parts_stock_history/parts_stock_history_Main.php');
    $menu->set_action('適正在庫計算', INDUST . 'reasonable_stock/reasonable_stock_calc_form.php');
