<?php
//////////////////////////////////////////////////////////////////////////////
// 受入検査の時間・件数の集計･分析 結果 照会             MVC Controller 部  //
// Copyright (C) 2006-2006 Kazuhiro.Kobayashi tnksys@nitto-kohki.co.jp      //
// Changed history                                                          //
// 2006/08/03 Created   acceptance_inspection_analyze_Controller.php        //
// 2006/11/30 outListLeadTime()受入検査日数の集計で初回リリース             //
// 2007/12/12 開始・終了日付の有効性チェック checkdate() を追加             //
//////////////////////////////////////////////////////////////////////////////
...
class AcceptanceInspectionAnalyze_Controller
{
    ///// Private properties
    ...
    ************************************************************************/
    ...
    {
        if ($this->error) {
            $this->model->outListErrorMessage($this->request, $this->menu);
            return;
        }
        switch ($this->request->get('Action')) {
        case 'ListLeadTime':                                // 担当者毎の検査日数のリスト生成
            $this->model->outListLeadTime($this->request, $this->menu);
            if ($this->request->get('showMenu') == '') $this->request->add('showMenu', 'List');
            break;
        case 'ListInspectionTime':                          // 担当者毎の受入検査時間リスト生成
            $this->model->outListInspectionTime($this->request, $this->menu);
            if ($this->request->get('showMenu') == '') $this->request->add('showMenu', 'List');
            break;
        case 'ListDetails':                                 // 担当者毎の明細リスト生成(リストやグラフから2次的に呼出し)
            $this->model->outListDetails($this->request, $this->menu);
            if ($this->request->get('showMenu') == '') $this->request->add('showMenu', 'ListWin');
            break;
        case 'GraphLeadTime':                               // 担当者毎の検査日数のグラフ生成
            $this->model->outGraphLeadTime($this->request, $this->result);
            if ($this->request->get('showMenu') == '') $this->request->add('showMenu', 'Graph');
            break;
        case 'GraphInspectionTime':                         // 担当者毎の受入検査時間グラフ生成
            $this->model->outGraphInspectionTime($this->request, $this->result);
            if ($this->request->get('showMenu') == '') $this->request->add('showMenu', 'Graph');
            break;
