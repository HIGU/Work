    ...
    if (getCheckAuthority(22)) {                    // 認証チェック
        ...
        if ( getResult($query, $res_chk) > 0 ) {    // 登録あり UPDATE 更新
            ...
            if (query_affected($query) <= 0) {
                $_SESSION['s_sysmsg'] .= "製品番号：{$parts_no} の掛率を{$credit_per}へ変更失敗！";      // .= に注意
                $msg_flg = 'alert';
                return false;
            } else {
                $_SESSION['s_sysmsg'] .= "製品番号：{$parts_no}の 掛率を{$credit_per}に変更しました！"; // .= に注意
                return true;
            }
        } else {                                    // 登録なし INSERT 新規   
            ...
            if (query_affected($query) <= 0) {
                $_SESSION['s_sysmsg'] .= "製品番号：{$parts_no} 掛率：{$credit_per}の追加に失敗！";      // .= に注意
                $msg_flg = 'alert';
                return false;
            } else {
                $_SESSION['s_sysmsg'] .= "製品番号：{$parts_no} 掛率：{$credit_per}を追加しました！";    // .= に注意
                return true;
            }
        }
    } else {                                        // 権限なしエラー
...
function productMaster_del($request)
{
    if (getCheckAuthority(22)) {    // 認証チェック
        ...
        if (query_affected($query) <= 0) {
            $_SESSION['s_sysmsg'] .= "製品番号：{$parts_no} 掛率：{$credit_per}の削除に失敗！";   // .= に注意
            $msg_flg = 'alert';
            return false;
        } else {
            $_SESSION['s_sysmsg'] .= "製品番号：{$parts_no} 掛率：{$credit_per}を削除しました！"; // .= に注意
            return true;
        }
    } else {                        // 権限なしエラー
        $_SESSION['s_sysmsg'] .= "編集権限がありません。必要な場合には、担当者に連絡して下さい。";
...
{
    $query_g = "
        SELECT  p.parts_no                  AS 製品番号     -- 0
            ,   trim(substr(m.midsc,1,30))  AS 製品名       -- 1
            ,   p.credit_per                AS 掛率         -- 2
        FROM
