<?php
//////////////////////////////////////////////////////////////////////////////
// 部品 在庫・有効利用数(予定在庫数)マイナスリスト照会  Client interface 部 //
// データ更新はparts_stock_history_master_ftp_cli.php  MVC Controller Main部//
...
require_once ('parts_stock_avail_minus_Controller.php');// MVC の Controller部
require_once ('parts_stock_avail_minus_Model.php');     // MVC の Model部
...
function main()
{
    ...
    ////////////// サイト設定
    $menu->set_site(INDEX_INDUST, 18);                  // site_index=INDEX_INDUST(生産メニュー) site_id=17(マイナスリスト)999(サイトを開く)
    ////////////// リターンアドレス設定
    // $menu->set_RetUrl(EQUIP_MENU);                   // 通常は指定する必要はない
    
    //////////// タイトル名(ソースのタイトル名とフォームのタイトル名)
    $title = '部品 在庫・有効利用数(予定在庫数)マイナスリスト照会';
