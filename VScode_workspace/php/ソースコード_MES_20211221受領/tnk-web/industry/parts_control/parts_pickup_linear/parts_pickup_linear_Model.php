...
//            テーブル名も上記と同様に変更 parts_pickup_linear 新規作成     //
// 2006/06/14 インボイスデータの取込処理を追加 メソッド invoiceTOplan()     //
// 2006/06/15 インボイスNo.入力時に自動計画入力メソッドを追加invoicePlanAdd //
...
class PartsPickupLinear_Model extends ComTableMnt
{
    ...
    public function table_add($plan_no, $user_id)
    {
        if ($this->pickupAuthUser()) {
            ...
            if ($this->getUniResult($chk, $check) <= 0) {   // 計画番号が登録されているか
                if (!$this->invoicePlanAdd($plan_no, $user_id)) {   // インボイス番号による連続計画入力
                    return false;
                }
                return true;
            } else {
    ...
    ***************************************************************************/
    ...
    protected function invoiceTOplan()
    {
        ...
        $fp = fopen($file, 'r');
        while (!(feof($fp))) {
            $data = fgetcsv($fp, 50, '_');     // 実レコードは38バイトなのでちょっと余裕をデリミタは'_'に注意
            if (feof($fp)) {
                break;
            }
            if ($data[0] == '') continue;
        ...
        if ( ($rows=$this->getResult2($query, $res)) >= 1) {
            for ($i=0; $i<$rows; $i++) {
                if (!$this->table_add_check($res[$i][0], $user_id)) {
                    return false;
                    // $_SESSION['s_sysmsg'] = 'インボイスNo.からの自動計画入力で入力できない計画がありました。';
                }
            }
            return true;
        }
