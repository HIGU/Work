...
// Changed history                                                          //
// 2005/09/12 Created    parts_pickup_linear.js                             //
...
/****************************************************************************
/*         parts_pickup_linear class テンプレートの拡張クラスの定義         *
/****************************************************************************
class parts_pickup_linear extends base_class
{   */
    ///// スーパークラスの継承
    parts_pickup_linear.prototype = new base_class();   // base_class の継承
    ///// グローバル変数 _GDEBUG の初期値をセット(リリース時はfalseにセットする)
    var _GDEBUG = false;
    
    ///// Constructer の定義
    function parts_pickup_linear()
    {
        /***********************************************************************
        *                           Private properties                         *
        ***********************************************************************/
        ...
        *                           Public methods                              *
        ************************************************************************/
        parts_pickup_linear.prototype.set_focus           = set_focus;            // 指定の入力エレメントにフォーカス
        parts_pickup_linear.prototype.blink_disp          = blink_disp;           // 点滅表示メソッド
        parts_pickup_linear.prototype.obj_upper           = obj_upper;            // オブジェの値を大文字変換
        parts_pickup_linear.prototype.win_open            = win_open;             // サブウィンドウを中央に表示
        parts_pickup_linear.prototype.winActiveChk        = winActiveChk;         // サブウィンドウのActiveチェック
        parts_pickup_linear.prototype.win_show            = win_show;             // モーダルダイアログを表示(IE専用)
        parts_pickup_linear.prototype.start_formCheck     = start_formCheck;      // start_form の入力チェックメソッド
        parts_pickup_linear.prototype.user_formCheck      = user_formCheck;       // user_form の入力チェックメソッド
        parts_pickup_linear.prototype.ControlFormSubmit   = ControlFormSubmit;    // ControlForm のサブミットメソッド
...
/*
}   // class parts_pickup_linear END  */


///// インスタンスの生成
var PartsPickupLinear = new parts_pickup_linear();
