<?php 
//////////////////////////////////////////////////////////////////////////////
// 設備稼働管理システムの機械運転日報 ロジックファイル                      //
// Copyright (C) 2004-2021 Kazuhiro.Kobayashi tnksys@nitto-kohki.co.jp      //
// Original by yamagishi@matehan.co.jp                                      //
// Changed history                                                          //
// 2004/07/15 Created  ReportEntry.php                                      //
// 2006/04/20 ReadDataバー材の場合の投入重量を小数点２桁及び端材の算出変更  //
// 2006/04/21 ReadData()に更に累計投入数・累計投入重量を追加                //
//            EDITモード時に$Report['Type']をパラメータで渡す。切断材の対応 //
// 2006/04/26 setParameter()に$Report['Type']=trim(@$_REQUEST['Type']) 追加 //
// 2007/03/28 equip_uploadテーブルのレイアウト変更及び時間表記をAS/400に調整//
//                  equip_upload_summaryに本稼働時間(自動+無人)カラムを追加 //
//            日報の再確定対応のため確定時の書込み時に旧データ削除処理追加  //
//            equip_upload_summaryに投入材料コードと材料重量(個数)を追加    //
// 2007/04/04 日報修正時の作業時間マイナスのチェックロジックを追加          //
//            equip_upload_summaryの$Summary['plan_num']→$Report['Plan']へ //
//            ($Report['Today']-$Report['Ng']) → $Report['Today'] へ変更   //
// 2007/04/11 手入力対応でsetParameter()のSQLとReadData()のSQL文を変更      //
// 2010/05/25 スイッチの10、11をそれぞれ計画停止と優先対応停止として使用    //
//            するための変更（AS側は刃物切れ時間４と一時停止時間５）   大谷 //
// 2021/08/02 SUSライン（fact=8）は確定時にASへ送らないように一時変更  大谷 //
// 2021/08/19 登録時に1440分を超えていた場合、エラーにするよう変更     大谷 //
// 2021/10/04 SUSライン（fact=8）も確定時にASへ送るように変更               //
//            fact=6にして、6工場組立自動機に変更                      大谷 //
//////////////////////////////////////////////////////////////////////////////
...
function setParameter()
{
    ...
    $Report['SijiNo']           = trim (@$_REQUEST['SijiNo']);
    ...
    if (is_numeric($Report['MacNo']) && is_numeric($Report['SijiNo']) && is_numeric($Report['KouteiNo'])) {
        ...
        $sql = " select "
             ...
             . " WHERE a.inst_no=" . pg_escape_string ($Report['SijiNo']) . " and b.koutei=" . pg_escape_string ($Report['KouteiNo']);
        ...
        if ($row = pg_fetch_array ($rs)) {
            ...
            $Report['SijiNum']       = $row['inst_qt'];
        }
    }
    
    // 行程記号の取得（仮）
    $Report['KouteiName'] = '';
    if (is_numeric($Report['SijiNo']) && is_numeric($Report['KouteiNo'])) {
        $sql = 'select pro_mark from equip_work_instruction where inst_no=' . pg_escape_string ($Report['SijiNo']) . ' and koutei=' . pg_escape_string ($Report['KouteiNo']);
        $rs = pg_query($con,$sql);
        if ($row = pg_fetch_array ($rs)) {
...
function EntryDataCheck()
{
    ...
    if (@$_REQUEST['ErrorCheckLevel'] == 1) {
        ...
        if ($Report['SijiNo'] == '') {
            ...
            if (!is_numeric($Report['SijiNo'])) {
                $Report['ENTRY_LEVEL'] = '1';
                $Message .= '指示No.は数値で入力して下さい。\n\n';
            } else {
                // 部品No.が取得できていなければマスタに存在しない
                if (!isset ($Report['ItemCode']) && $Report['SijiNo'] != CUSTOM_MADE_SIJI_NO) {
                    // 入力情報レベル１（キー入力レベル）
                    $Report['ENTRY_LEVEL'] = '1';
                    $Message .= '指示No.'.$Report['SijiNo'].'はマスタに登録されていません。\n\n';
                }
            }
        }
        // 行程番号
        ...
        if ($Message == '') {
            $sql = "select work_date from equip_work_report where work_date=" . pg_escape_string ($Report['WorkDate']) . " and mac_no=" . pg_escape_string ($Report['MacNo']) . " and siji_no=" . pg_escape_string ($Report['SijiNo']) . " and koutei=" . pg_escape_string ($Report['KouteiNo']);
            $rs  = pg_query ($con,$sql);
            if ($row = pg_fetch_array ($rs)) {
                // 入力情報レベル１（キー入力レベル）
                $Report['ENTRY_LEVEL'] = '1';
    ...
    if (@$_REQUEST['ErrorCheckLevel'] == 2) {
        ...
        for ($i=0;$i<$LogNum;$i++) {
            ...
            if ($isEntry == 5) {
                ...
                $report_time = CalWorkTime($Report['FromDate'][$i],$Report['FromTime'][$i],$Report['ToDate'][$i],$Report['ToTime'][$i]) - $Report['CutTime'][$i];
                $report_time_t = $report_time_t + $report_time;
                if($report_time_t > 1440) {
                    $Message .= '合計の作業時間が1440分を超えています。\n\n';
                }
            }
        }
        // 材料コード
...
function SaveData()
{
    ...
    if (@$_REQUEST['EDIT_MODE'] == 'UPDATE') {
        // 日報ヘッダ削除
        $sql = "delete from equip_work_report where work_date=" . pg_escape_string ($Report['WorkDate']) . " and mac_no=" . pg_escape_string ($Report['MacNo']) . " and siji_no=" . pg_escape_string ($Report['SijiNo']) . " and koutei=" . pg_escape_string ($Report['KouteiNo']);
        if (!pg_query ($con , $sql)) {
        ...
        // 運転ログ削除
        $sql = "delete from equip_work_report_log where work_date=" . pg_escape_string ($Report['WorkDate']) . " and mac_no=" . pg_escape_string ($Report['MacNo']) . " and siji_no=" . pg_escape_string ($Report['SijiNo']) . " and koutei=" . pg_escape_string ($Report['KouteiNo']);
        if (!pg_query ($con , $sql)) {
            pg_query ($con , 'ROLLBACK');
            $SYSTEM_MESSAGE = "データベースの更新に失敗しました\n$sql";
            require_once ('../com/' . ERROR_PAGE);
    ...
    // 日報ヘッダ作成
    $sql = "insert into equip_work_report (work_date,mac_no,siji_no,koutei,yesterday,today,end_flg,ng,ng_kbn,plan,memo,injection_item,injection,abandonment,decision_flg,last_user) values ( "
         .    pg_escape_string ($Report['WorkDate'])        ." ,"
         .    pg_escape_string ($Report['MacNo'])           ." ,"
         .    pg_escape_string ($Report['SijiNo'])          ." ,"
    ...
    $sql    = "insert into equip_work_report_log(work_date,mac_no,siji_no,koutei,mac_state,from_date,from_time,to_date,to_time,cut_time,last_user) ";
    for ($i=0;$i<$LogNum;$i++) {
        // データがないレコードは無視
...
function ReadData()
{
    ...
    $sql = "select "
         ...
         . "    c.delivery          as delivery,        "
         . "    c.inst_qt           as inst_qt,         "
         . "    c.parts_no          as parts_no,        "
         . "    c.inst_qt           as plan_cnt,        "
         . "    e.midsc             as midsc,           "
         . "    e.mzist             as mzist            "
         . "from equip_work_report a "
         ...
         . "left outer join equip_work_inst_header c on a.siji_no=c.inst_no "
         . "left outer join equip_work_log2_header d on a.mac_no=d.mac_no and a.siji_no=d.siji_no and a.koutei=d.koutei " 
         ...
         . "where work_date=".pg_escape_string ($Report['WorkDate'])." and a.mac_no=".pg_escape_string ($Report['MacNo'])." and a.siji_no=".pg_escape_string ($Report['SijiNo'])." and a.koutei=".pg_escape_string ($Report['KouteiNo']);
    ...
    if ($row = pg_fetch_array ($rs)) {
        ...
        $Report['SijiNo']        = trim ( $row['siji_no'] );
        ...
        $Report['SijiNum']       = trim ( $row['plan_cnt'] );
        ...
        $query = "SELECT sum(injection) AS sum_injection FROM equip_work_report WHERE work_date<={$row['work_date']} AND mac_no={$row['mac_no']} AND siji_no={$row['siji_no']} AND koutei={$row['koutei']}";
    ...
    // 日報運転ログ読み込み
    // --------------------------------------------------
    $sql = " select work_date,mac_no,siji_no,koutei,mac_state,from_date,from_time,to_date,to_time,cut_time from equip_work_report_log "
         . " where work_date=".pg_escape_string ($Report['WorkDate'])." and mac_no=".pg_escape_string ($Report['MacNo'])." and siji_no=".pg_escape_string ($Report['SijiNo'])." and koutei=".pg_escape_string ($Report['KouteiNo'])
...
function ExecuteDecision()
{
    ...
    $fact = '';
    $sql  = "select factory from equip_machine_master2 where mac_no=" . pg_escape_string ($Report['MacNo']);
    $rs   = pg_query ($con , $sql);
    if (!$row = pg_fetch_array ($rs)) {
    } else {
        $fact = $row['factory'];
    ...
    if ($fact<>'6') {   // SUS日報登録時にコメント化せずに 6工場に変更
