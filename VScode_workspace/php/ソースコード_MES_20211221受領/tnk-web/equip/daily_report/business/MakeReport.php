<?php 
//////////////////////////////////////////////////////////////////////////////
// 設備稼働管理システムの機械運転日報 日報作成ビジネスロジック              //
// Copyright (C) 2004-2006 Kazuhiro.Kobayashi tnksys@nitto-kohki.co.jp      //
// Original by yamagishi@matehan.co.jp                                      //
// Changed history                                                          //
// 2004/07/15 Created  MakeReport.php                                       //
// 2004/09/27 工場別運転日報に対応 及び equip_index2()関数インデックス追加  //
// 2005/02/23 .LOCKファイルを工場毎に分散し、他の工場の影響を受けないように //
//            最大実行時間を20→40へ変更(１工場のため)将来的にはロジックを変//
// 2006/04/20 バー材の場合の投入重量を小数点２桁対応及び端材の算出変更      //
//            MakeHeaderRecode() の変更                                     //
// 2006/06/12 equip_partsを部品番号と機械番号の複合キーへ変更したため       //
//            ２箇所のロジック変更とgetPartsMaster()を最後に追加            //
//////////////////////////////////////////////////////////////////////////////
...
function MakeMachineReportOneDay($MacNo,$CsvFlg,$Day)
{
    ...
    $sql = " SELECT "
         ...
         . "                       mac_no, "
         . "                       siji_no "
         . "                   ) b on a.mac_no=b.mac_no and a.siji_no=b.siji_no "
         . " WHERE "
         // . "     a.mac_no=$MacNo AND "
    ...
    while (true) {
        ...
        if ($KeyCode >= 2) {
            ...
            if ($NowRow['siji_no'] == $StartSijiNo && mu_Date::toString($NowRow['date_time'],"Hi") != BUSINESS_DAY_CHANGE_TIME) {
                ...
                $basicDate = REPORT_START_DATE . BUSINESS_DAY_CHANGE_TIME;
                ...
                if ($logrow =  pg_fetch_array ($logrs)) {
                    ...
                    if ($logrow['mac_state'] == $NowRow['mac_state']) {
                        ...
                        if ($logrow['siji_no'] != $NowRow['siji_no']) {
                            $Report['mac_no']    = $MacNo;
                            $Report['siji_no']   = $logrow['siji_no'];
