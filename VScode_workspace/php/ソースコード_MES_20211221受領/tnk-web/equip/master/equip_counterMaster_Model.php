...
// Changed history                                                          //
// 2005/07/15 Created   equip_counterMaster_Model.php                       //
...
/******************************************************************************
* 機械のワークカウンターマスター用 MVCのModel部の base class 基底クラスの定義 *
******************************************************************************/
class EquipCounterMaster_Model extends ComTableMnt
{
    ///// Private properties
    private $factory;
    ...
    ************************************************************************/
    ...
    public function __construct($factory='', $request)
    {
        ...
        parent::__construct($sql_sum, $request, 'equip_counterMaster.log');
    }
    
    ////////// マスター追加
    public function table_add($mac_no, $parts_no, $count)
    {
        if (equipAuthUser('FNC_MASTER')) {
            $chk_sql1 = "select mac_no from equip_count_master where mac_no={$mac_no} and parts_no='{$parts_no}' limit 1";
            if ($this->getUniResult($chk_sql1, $check) > 0) {    // mac_no & parts_noの登録済みのチェック
                ...
                $response = $this->add_execute($mac_no, $parts_no, $count);
                if ($response) {
                    return true;
                } else {
    ...
    ////////// マスター 変更
    public function table_change($preMac_no, $preParts_no, $mac_no, $parts_no, $count)
    {
        if (equipAuthUser('FNC_MASTER')) {
            $query = "select mac_no from equip_count_master where mac_no={$preMac_no} and parts_no='{$preParts_no}'";
            if ($this->getUniResult($query, $check) > 0) {  // 変更前の機械番号と部品番号が登録されているか？
                $chk_sql1 = "select mac_no from equip_count_master where mac_no={$mac_no} and parts_no='{$parts_no}'";
                if ( ($preMac_no != $mac_no) || ($preParts_no != $parts_no) ) {
                    if ($this->getUniResult($chk_sql1, $check) > 0) {    // 変更後の機械番号と部品番号が既に登録されているか？
                        $_SESSION['s_sysmsg'] = "機械番号:{$mac_no} 部品(製品)番号:{$parts_no} は既に登録されています！";
                    } else {
                        $response = $this->chg_execute($preMac_no, $preParts_no, $mac_no, $parts_no, $count);
                        if ($response) {
                            return true;
                        } else {
    ...
    {
        if (equipAuthUser('FNC_MASTER')) {
            $chk_sql = "select mac_no from equip_count_master where mac_no={$mac_no} and parts_no='{$parts_no}'";
    ...
    {
        $query = "SELECT cnt.mac_no
                        ,substr(mac_name, 1, 10)
                        ,cnt.parts_no
                        ,substr(midsc, 1, 20)
                        ,cnt.count
                        ,to_char(cnt.regdate AT TIME ZONE 'JST', 'YYYY/MM/DD HH24:MI')
                        ,to_char(cnt.last_date AT TIME ZONE 'JST', 'YYYY/MM/DD HH24:MI')
                    FROM
                        equip_count_master      AS cnt
                    LEFT OUTER JOIN
    ...
    ***************************************************************************/
    ...
    private function add_execute($mac_no, $parts_no, $count)
    {
        ...
        $insert_qry = "
            insert into equip_count_master
            (mac_no, parts_no, count, last_date, last_user)
            values
            ($mac_no, '$parts_no', $count, '$last_date', '$last_user')
        ";
    ...
    private function chg_execute($preMac_no, $preParts_no, $mac_no, $parts_no, $count)
    {
        // 保存用のSQL文を設定
        $save_sql = "select * from equip_count_master where mac_no={$preMac_no} and parts_no='{$preParts_no}'";
        // ここに last_date last_user の登録処理を入れる
        $last_date = date('Y-m-d H:i:s');
        $last_user = $_SESSION['User_ID'];
        $update_sql = "
            update equip_count_master set
            mac_no={$mac_no}, parts_no='{$parts_no}', count={$count}, last_date='{$last_date}', last_user='{$last_user}'
    ...
    {
        // 保存用のSQL文を設定
        $save_sql   = "select * from equip_count_master where mac_no={$mac_no} and parts_no='{$parts_no}'";
        $delete_sql = "delete from equip_count_master where mac_no={$mac_no} and parts_no='{$parts_no}'";
        // $save_sqlはオプションなので指定しなくても良い
