...
// Changed history                                                          //
// 2001/07/07 Created   add_receiveentry.php                                //
// 2002/08/07 register_globals = Off 対応 & セッション管理                  //
// 2005/01/17 access_log の変更と view_file_name(__FILE__) の追加           //
// 2005/07/21 教育の一括登録で、途中の社員番号の抜けに対応                  //
// 2007/02/06 一括登録で空白の場合は飛ばして登録できるように対応 大谷       //
// 2007/02/08 登録可能者が居ないときのチェックを追加 大谷                   //
// 2007/02/09 空白の時古い社員番号が残ってしまう不具合を訂正 大谷           //
// 2007/02/15 POSTをREQUESTに変更                                           //
//            社員名の表示を追加    大谷                                    //
// 2007/07/06 登録時に教育名を表示するように変更    大谷                    //
...
if ($_SESSION['Auth'] < 2) {
    if ($_SESSION['User_ID'] != '970268' && $_SESSION['User_ID'] != '300551') {
        ...
        header('Location: ' . H_WEB_HOST . EMP . 'emp_menu.php?func=' . FUNC_CHGRECEIVE);
        exit();
    }
}

$rows = count($_REQUEST['uid']);
$_SESSION['s_sysmsg'] = "教育の登録　{$_REQUEST['receive_name']}\\n";
$i = 0; // 登録者数
for ($r=0; $r<$rows; $r++) {
    if (!$_REQUEST['uid'][$r] == '') {
...
if ($rows_uid > 0) {
    ...
    if (addReceiveentry($_REQUEST['uid'],$_REQUEST['receive'],$_REQUEST['begin_date'],$_REQUEST['end_date'])) {
        header("Location: http:" . WEB_HOST . "emp/emp_menu.php?func=" . FUNC_CHGRECEIVE);
        exit();
    }
    $_SESSION['s_sysmsg'] = "教育の追加に失敗しました。管理者にお問い合わせください。";
    header('Location: ' . H_WEB_HOST . EMP . 'emp_menu.php?func=' . FUNC_CHGRECEIVE);
} else {
