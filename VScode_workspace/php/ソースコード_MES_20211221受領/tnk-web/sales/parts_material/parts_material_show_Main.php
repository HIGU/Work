<?php
//////////////////////////////////////////////////////////////////////////////
// 部品売上げの材料費(購入費)の 照会                  Client interface 部   //
//                                              MVC Controller の Main 部   //
// Copyright (C) 2006-2006 Kazuhiro.Kobayashi tnksys@nitto-kohki.co.jp      //
// Changed history                                                          //
// 2006/02/15 Created   parts_material_show_Main.php                        //
//////////////////////////////////////////////////////////////////////////////
...
require_once ('parts_material_show_Controller.php');// MVC の Controller部
require_once ('parts_material_show_Model.php');     // MVC の Model部
...
function main()
{
    ...
    ////////////// サイト設定
    $menu->set_site(INDEX_SALES, 16);           // site_index=INDEX_INDUST(生産メニュー) site_id=16(部品売上の材料費)999(サイトを開く)
    ////////////// リターンアドレス設定
    // $menu->set_RetUrl(EQUIP_MENU);              // 通常は指定する必要はない
    
    //////////// タイトル名(ソースのタイトル名とフォームのタイトル名)
    $title = '部品売上の材料費(購入費) 照会';
    $menu->set_title($title);
    //////////// 呼出先のaction名とアドレス設定
    // $menu->set_action('引当構成表',   INDUST . 'material/allo_conf_parts_view.php');
    
    //////////// コントローラー部のインスタンス生成
    $controller = new PartsMaterialShow_Controller($menu);
