<?php
//////////////////////////////////////////////////////////////////////////////
// 少額資産台帳の ＰＤＦ出力(印刷) FPDF/MBFPDF 使用 (除却含む）             //
// Copyright (C) 2010 - 2012 Norihisa.Ohya norihisa_ooya@nitto-kohki.co.jp  //
// Changed history                                                          //
// 2010/10/19 Created  smallSum_assetsList_delyes.php  ゴシック体           //
// 2012/03/02 承認の押印欄を変更 品名の文字数を制限                         //
// 2012/10/03 除却済の品名がはみ出してしまうのを修正                        //
//            念のためメーカー・型式、備考にも文字数制限を追加              //
//////////////////////////////////////////////////////////////////////////////
...
class PDF_j extends MBFPDF  // 日本語PDFクラスを拡張します。
{
    ...
    function FancyTable($data, $caption)
    {
        ...
        $this->Cell($this->w_usr[0], 7, '', 'LTB', 0, 'L', 1);
        $this->Cell($this->w_usr[1], 7, '', 'TB', 0, 'L', 1);
        $this->SetTextColor(50, 0, 255);    // キャプションだけ色を変える(青)
        $this->Cell($this->w_usr[2], 7, $caption, 'TB', 0, 'R', 1);
        $this->Cell($this->w_usr[3], 7, '', 'TB', 0, 'L', 1);
        $this->Cell($this->w_usr[4], 7, '', 'TB', 0, 'L', 1);
        $this->Cell($this->w_usr[5], 7, '', 'TB', 0, 'L', 1);
        $this->Cell($this->w_usr[6], 7, '', 'TB', 0, 'L', 1);
        $this->Cell($this->w_usr[7], 7, '', 'RTB', 0, 'L', 1);
       $this->SetTextColor(0, 0, 0);
        $this->Ln();    // 改行
        ...
        foreach ($data as $row) {
            ...
            $this->Cell($this->w_usr[6], 7, $row[6], 'LRTB', 0, 'C', $fill);
            $this->Cell($this->w_usr[7], 7, "　", 'LRTB', 0, 'L', $fill);
            $this->Ln();
            $fill = !$fill;
        }
        $this->SetX(5);
    ...
    {
        $this->Image('/home/www/html/tnk-web/img/t_nitto_logo2.png', 235, 14, 50, 0, '', 'R');  //イメージを配置します。場所を指定します。→リファレンス参照
        $this->SetX(60);
...
// Header
// Column titles
$pdf->wh_usr = array('設置場所', '品　　名', 'メーカー・型式', '購入年月日', '購入金額', '備考', '除却年月日', '確認');
$pdf->w_usr  = array(36, 55, 60, 25, 20, 55, 25, 12);   //各セルの横幅を指定しています。
...
while ($row = pg_fetch_object($res_usr)) {
    ...
    $query = "SELECT set_place              AS placecode
                , substr(assets_name,0,21)  AS assetname
    ...
    ///// 出力 本文
    $pdf->FancyTable($data_f, '未　除　却');  // 上でカスタムしたメンバ関数を呼び出す
    
    /* 受講履歴を取得 SQL */
    $query = "SELECT set_place              AS placecode
                , substr(assets_name,0,13)  AS assetname
                , substr(assets_model,0,14) AS amodel
                , buy_ym                    AS abuyym
                , buy_price                 AS abuyprice
                , substr(note,0,20)         AS anote
    ...
    while ($rows = pg_fetch_object($res)) {
        ...
        $abuyym    = $pdf->format_date8($rows->abuyym);
        $abuyprice = number_format($rows->abuyprice);
        $adelym    = $pdf->format_date8($rows->adelym);
        $anote     = $rows->anote;
        $data_f[$cnt] = array($aplace, $assetname, $amodel, $abuyym, $abuyprice, $anote, $adelym);
        $cnt++;
    }
    //$pdf->AddPage('L','A4');    // ページを生成。最低1回はコールする必要がある(逆に$pdf->Open()は省略可能)
    //$pdf->AddPage();    // ページを生成。最低1回はコールする必要がある(逆に$pdf->Open()は省略可能)
    // $pdf->SetFont(GOTHIC, '', 12);
    ///// 出力 本文
    $pdf->FancyTable($data_f, '除　却　済');  // 上でカスタムしたメンバ関数を呼び出す
}
