//////////////////////////////////////////////////////////////////////////////
// 損益予測の集計・分析 結果 照会              MVC View部(JavaScriptクラス) //
// Copyright (C) 2011-     Norihisa.Ohya norihisa_ooya@nitto-kohki.co.jp    //
// Changed history                                                          //
// 2011/07/13 Created    profit_loss_estimate_view.js                       //
// 2011/08/02 印刷関数のテスト                                              //
// 2011/08/04 単位と桁数を追加                                              //
//////////////////////////////////////////////////////////////////////////////
...
/****************************************************************************
/*     profit_loss_estimate_view class base_class の拡張クラスの定義             *
/****************************************************************************
class profit_loss_estimate_view extends base_class
*/
///// スーパークラスの継承
profit_loss_estimate_view.prototype = new base_class();    // base_class の継承
///// Constructer の定義
function profit_loss_estimate_view()
{
    /***********************************************************************
    *                           Private properties                         *
    ***********************************************************************/
    ...
    ************************************************************************/
    ...
    profit_loss_estimate_view.prototype.set_focus = function (obj, status)
    {
        if (obj) {
            obj.focus();
    ...
    profit_loss_estimate_view.prototype.blink_disp = function (id_name)
    {
    ...
    /***** 点滅ストップメソッド *****/
    profit_loss_estimate_view.prototype.stop_blink = function ()
    {
    ...
    /***** オブジェクトの値を大文字変換する *****/
    profit_loss_estimate_view.prototype.obj_upper = function (obj)
    {
    ...
    profit_loss_estimate_view.prototype.win_open = function (url, w, h)
    {
        if (!w) w = 800;     // 初期値
        if (!h) h = 600;     // 初期値
    ...
    profit_loss_estimate_view.prototype.winActiveChk = function ()
    {
    ...
    profit_loss_estimate_view.prototype.win_show = function (url, w, h)
    {
        if (!w) w = 800;     // 初期値
        if (!h) h = 600;     // 初期値
    ...
    profit_loss_estimate_view.prototype.dateCreate = function (obj)
    {
    ...
    profit_loss_estimate_view.prototype.checkConditionForm = function (obj)
    {
        // obj.targetPartsNo.value = obj.targetPartsNo.value.toUpperCase();
        if (!obj.targetDateYM.value) {
            alert("対象年月が選択されていません！");
            obj.targetDateYM.focus();
            obj.targetDateYM.select();
            return false;
        }
        if (!obj.targetDateYMD.value) {
            alert("作成日付が選択されていません！");
            obj.targetDateYMD.focus();
            obj.targetDateYMD.select();
            return false;
        }
        // return false;   // デバッグ中
        /************
        ...
        ************/
        ...
        this.parameter += "&targetDateYMD=" + obj.targetDateYMD.value;
        this.parameter += "&keihi_tani=" + obj.keihi_tani.value;
        this.parameter += "&keihi_keta=" + obj.keihi_keta.value;
        return true;
    }
    /***** ConditionForm の入力チェックをしてAjax実行 *****/
    profit_loss_estimate_view.prototype.checkANDexecute = function (obj, flg)
    {
     ...
     /***** ConditionForm の入力チェックをしてReload実行 *****/
    profit_loss_estimate_view.prototype.checkANDReload = function (obj, flg)
    {
        var parm = "?";
        parm += "targetDateYM=" + obj.targetDateYM.value;
        location.href ="profit_loss_estimate_view_Main.php" + parm;
        return false;   // 実際にsubmitはさせない
    }
    ...
    profit_loss_estimate_view.prototype.AjaxLoadTable = function (showMenu, location)
    {
        ...
        if (showMenu == "ListWin") {    // 別ウィンドウで表示
            this.win_open("profit_loss_estimate_view_Main.php"+parm, 700, 350);
            return;
        }
        /***
        ...
        ***/
        ...
        try {
            xmlhttp.open("GET", "profit_loss_estimate_view_Main.php"+parm);
            xmlhttp.send(null);
        } catch (e) {
            alert(url + "\n\nをオープン出来ません！\n\n" + e);
        }
    ...
    /***** 結果表示領域のクリアーメソッド *****/
    profit_loss_estimate_view.prototype.viewClear = function ()
    {
    ...
    profit_loss_estimate_view.prototype.WaitMessage = function ()
    {
...
}   /* class profit_loss_estimate_view END  */


///// インスタンスの生成
var ProfitLossEstimateView = new profit_loss_estimate_view();
