//////////////////////////////////////////////////////////////////////////////
// 会社の基本カレンダー メンテナンス           MVC View部(JavaScriptクラス) //
// Copyright (C) 2006-2006 Kazuhiro.Kobayashi tnksys@nitto-kohki.co.jp      //
// Changed history                                                          //
// 2006/06/20 Created   companyCalendar.js                                  //
// 2006/06/29 AjaxLoadUrl(url)メソッドを追加                                //
// 2006/07/06 setTimeValue(),checkTimeValue()メソッド追加 initFormat()修正  //
...
/****************************************************************************
/*          companyCalendar class base_class の拡張クラスの定義             *
/****************************************************************************
class companyCalendar extends base_class
*/
///// スーパークラスの継承
companyCalendar.prototype = new base_class();    // base_class の継承
///// Constructer の定義
function companyCalendar()
{
    ...
    ************************************************************************/
    ...
    companyCalendar.prototype.set_focus = function (obj, status)
    {
        if (obj) {
            obj.focus();
    ...
    companyCalendar.prototype.blink_disp = function (id_name)
    {
    ...
    /***** 点滅ストップメソッド *****/
    companyCalendar.prototype.stop_blink = function ()
    {
    ...
    /***** オブジェクトの値を大文字変換する *****/
    companyCalendar.prototype.obj_upper = function (obj)
    {
    ...
    companyCalendar.prototype.win_open = function (url, w, h, winName)
    {
        if (!winName) winName = "";
        if (!w) w = 800;     // 初期値
        if (!h) h = 600;     // 初期値
    ...
    companyCalendar.prototype.winActiveChk = function ()
    {
    ...
    companyCalendar.prototype.win_show = function (url, w, h)
    {
        if (!w) w = 800;     // 初期値
        if (!h) h = 600;     // 初期値
    ...
    companyCalendar.prototype.dateCreate = function (obj, flg, offset)
    {
    ...
    /***** カレンダーのアクション切替メソッド *****/
    companyCalendar.prototype.setTargetCalendar = function (obj, flg, status)
    {
        if (!obj) return;     // parameterのチェック
        switch (status) {
        case 'BDSwitch':
    ...
    /***** カレンダーのアクション名 切替メソッド *****/
    companyCalendar.prototype.actionNameSwitch = function ()
    {
        if (document.ConditionForm.SetTime) {
    ...
    /***** 対象期のカレンダーを初期化します *****/
    companyCalendar.prototype.initFormat = function (obj, flg)
    {
        if (!obj) return;     // parameterのチェック
    ...
    companyCalendar.prototype.setTimeValue = function (formObj, targetObj)
    {
    ...
    companyCalendar.prototype.checkTimeValue = function (timeValue, formObj)
    {
        if (timeValue <= 0) {
    ...
    companyCalendar.prototype.checkConditionForm = function (obj)
    {
    ...
    companyCalendar.prototype.checkANDexecute = function (obj, flg)
    {
    ...
    companyCalendar.prototype.AjaxLoadTable = function (showMenu, location)
    {
        ...
        if (showMenu == "ListWin") {    // 別ウィンドウで表示
            this.win_open("companyCalendar_Main.php"+parm, 700, 350);
            return;
        }
        /***
        ...
        ***/
        ...
        var url = "companyCalendar_Main.php" + parm;
        try {
    ...
    /***** URL指定 Ajax用ロードメソッド *****/
    companyCalendar.prototype.AjaxLoadUrl = function (url)
    {
    ...
    /***** 結果表示領域のクリアーメソッド *****/
    companyCalendar.prototype.viewClear = function ()
    {
    ...
    companyCalendar.prototype.WaitMessage = function ()
    {
