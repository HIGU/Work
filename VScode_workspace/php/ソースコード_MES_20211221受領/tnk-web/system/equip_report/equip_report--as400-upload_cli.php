...
<?php
//////////////////////////////////////////////////////////////////////////////
// 機械運転日報(製造課) データ 自動FTP UPLOAD CLI版                         //
...
// 2007/04/11 明細・サマリーの件数をログに追加    CLI版を作成               //
...
// 2007/08/31 AS/400とのFTPエラー回避のためftpPutCheckAndExecute()を追加    //
//////////////////////////////////////////////////////////////////////////////
...
if ( ($rows=getResultTrs($con, $query, $res)) < 1) {
    ...
    for ($i=0; $i<$rows; $i++) {
        ...
        if (fwrite($fp, $log_record) == FALSE) {
            $log_date = date('Y-m-d H:i:s');            ///// 日報用ログの日時
            fwrite($fpa, "$log_date 明細のUPLOADデータ生成 error \n");
            fclose($fpa);   ///// 日報用ログファイルのクローズ
            query_affected_trans($con, 'ROLLBACK');
            exit();
        }
...
if (file_exists(UPLOAD_F1) && file_exists(UPLOAD_F2)) {
    ...
    $log_date = date('Y-m-d H:i:s');            ///// 日報用ログの日時
    fwrite($fpa,"$log_date 機械運転日報 明細又はサマリー 日報ファイルがありません。\n");
}
