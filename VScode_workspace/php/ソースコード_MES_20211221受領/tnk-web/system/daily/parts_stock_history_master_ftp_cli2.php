...
// 生産用 部品在庫 経歴(history) マスター 手動更新用 cli版                  //
// AS/400 ----> Web Server (PHP)                                            //
// Copyright (C) 2004-2005 Kazuhiro.Kobayashi tnksys@nitto-kohki.co.jp      //
// Changed history                                                          //
// 2004/12/20 Created  parts_stock_history_master_ftp_cli2.php              //
...
$file_debug  = '/home/guest/daily/debug/debug-HIZHST2.TXT';
if (file_exists($file_orign)) {         // ファイルの存在チェック
    ...
    fclose($fp);
    fclose($fpw);       // debug
    fwrite($fpa, "$log_date 在庫経歴2の更新 : $rec_ok/$rec 件登録しました。\n");
    fwrite($fpa, "$log_date 在庫経歴2の更新 : {$ins_ok}/{$rec} 件 追加 \n");
    fwrite($fpa, "$log_date 在庫経歴2の更新 : {$upd_ok}/{$rec} 件 変更 \n");
    echo "$log_date 在庫経歴2の更新 : $rec_ok/$rec 件登録しました。\n";
    echo "$log_date 在庫経歴2の更新 : {$ins_ok}/{$rec} 件 追加 \n";
    echo "$log_date 在庫経歴2の更新 : {$upd_ok}/{$rec} 件 変更 \n";
} else {
