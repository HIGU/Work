...
<?php
//////////////////////////////////////////////////////////////////////////////
// 単価レート区分マスター自動FTP Download cron で処理用コマンドライン版     //
// AS/400 ----> Web Server (PHP)                                            //
// Copyright (C) 2013-2013 Norihisa.Ohya norihisa_ooya@nitto-kohki.co.jp    //
// AS UKSLIB/QCLSRC \TNKDAILYCのLOOPの前に以下を登録すること                //
// SNDF       RCDFMT(TITLE)                                                 //
// SNDF       RCDFMT(RATEDIV)                                               //
// RUNQRY     QRY(UKPLIB/Q#RATEDIV)                                         //
// \FTPTNK    USER(AS400) ASFILE(W#RATEDIV) PCFILE(W#RATEDIV.CSV) MODE(CSV) //
// Changed histoy                                                           //
// 2013/05/27 Created  parts_ratediv_master_update_ftp.php                  //
...
fwrite($fpb, "単価レート区分マスターの更新\n");
fwrite($fpb, "/home/www/html/tnk-web/system/daily/parts_ratediv_master_update_ftp.php \n");
...
// 単価レート区分 日報処理 準備作業
$file_name  = '/home/guest/daily/W#RATEDIV.CSV';
$file_temp  = '/home/guest/daily/Q#RATEDIV.tmp';
$file_write = '/home/guest/daily/Q#RATEDIV.txt';
...
if (file_exists($file_name)) {            // ファイルの存在チェック
    ...
    while (FALSE !== ($data = fgetcsv($fp, 300, ',')) ) {    // CSV file として読込み
        ...
        fwrite($fpw,"{$data[0]}\t{$data[1]}\t{$data[2]}\t{$data[3]}\n");
...
// 単価レート区分マスター 日報処理
$file_name = '/home/guest/daily/Q#RATEDIV.txt';
$file_name_bak = '/home/guest/daily/Q#RATEDIV-bak.txt';
if (file_exists($file_name)) {            // ファイルの存在チェック
    ...
    if ( ($fp = fopen($file_name, 'r')) ) {
        ...
        // while ($data = fgetcsv($fp, 200, "_")) {     // FTP接続用
            ...
            ///////// 登録済みのチェック
            $query_chk = sprintf("select rate_sign from rate_div_master where rate_div='%s'", $data[0]);
            if (getUniResTrs($con, $query_chk, $res_chk) <= 0) {    // トランザクション内での 照会専用クエリー
                ///// 登録なし insert 使用
                $query = sprintf("insert into rate_div_master (rate_div, rate_sign, rate_name, rev_par)
                        values('%s','%s','%s',%f)", $data[0],$data[1],$data[2],$data[3]);
                ...
                ///// 登録あり update 使用
                $query = sprintf("update rate_div_master set rate_sign='%s', rate_name='%s', rev_par=%f
                        where rate_div='%s'", $data[1], $data[2], $data[3], $data[0]);
                if (query_affected_trans($con, $query) <= 0) {      // 更新用クエリーの実行
        ...
        echo "Q#RATEDIV.txtをオープン出来ません\n";
        fwrite($fpa,"".$rowcsv."W#RATEDIV.txtをオープン出来ません\n");
        fwrite($fpb,"".$rowcsv."W#RATEDIV.txtをオープン出来ません\n");
    }
    /**********
...
// メッセージを返す
if ($flag2==1) {
    echo "$log_date 単価レート区分マスターの更新: $rec_ok/$rowcsv 件登録しました。\n";
    echo "$log_date 単価レート区分マスターの更新: {$row_in}/{$rowcsv} 件 追加 \n";
    echo "$log_date 単価レート区分マスターの更新: {$row_up}/{$rowcsv} 件 変更 \n";
    fwrite($fpa, "$log_date 単価レート区分マスターの更新: $rec_ok/$rowcsv 件登録しました。\n");
    fwrite($fpa, "$log_date 単価レート区分マスターの更新: {$row_in}/{$rowcsv} 件 追加 \n");
    fwrite($fpa, "$log_date 単価レート区分マスターの更新: {$row_up}/{$rowcsv} 件 変更 \n");
    fwrite($fpb, "$log_date 単価レート区分マスターの更新: $rec_ok/$rowcsv 件登録しました。\n");
    fwrite($fpb, "$log_date 単価レート区分マスターの更新: {$row_in}/{$rowcsv} 件 追加 \n");
    fwrite($fpb, "$log_date 単価レート区分マスターの更新: {$row_up}/{$rowcsv} 件 変更 \n");
} else {
    echo "{$log_date} 単価レート区分マスターの更新データがありません。\n";
    fwrite($fpa, "$log_date 単価レート区分マスターの更新データがありません。\n");
    fwrite($fpb, "$log_date 単価レート区分マスターの更新データがありません。\n");
}
/////////// commit トランザクション終了
